...
fn writeMessage(ctx: *const CodegenContext, message: *const schema.MessageDescriptor, writer: *std.Io.Writer, depth: usize) Error!void {
    ...
    for (message.fields.items) |*field| {
        if (field.kind == .map) {
            ...
            try writeMapEntryHelpers(field, writer, depth + 1);
        }
    }
    ...
    try writer.writeAll("\n");
    try writeDeinit(file, message, writer, depth + 1);
    try writer.writeAll("\n");
...
fn writeOneofUnion(file: *const schema.FileDescriptor, message: *const schema.MessageDescriptor, oneof: schema.OneofDescriptor, writer: *std.Io.Writer, depth: usize) Error!void {
    try indent(writer, depth);
    try writer.writeAll("pub const ");
    ...
    for (message.fields.items) |*field| {
        if (field.oneof_name) |name| {
            if (std.mem.eql(u8, name, oneof.name)) {
                ...
                try writer.writeAll(": ");
                if (typedOneofMessageField(file, field)) |type_name| {
...
fn writeFieldMetadataDecl(ctx: *const CodegenContext, field: *const schema.FieldDescriptor, writer: *std.Io.Writer, depth: usize) Error!void {
    ...
    if (typedSingularMessageField(file, field)) |type_name| {
...
fn writeTextParseMessagePayloadAssign(ctx: *const CodegenContext, field: *const schema.FieldDescriptor, type_name: []const u8, writer: *std.Io.Writer, depth: usize) Error!void {
    ...
    try indent(writer, depth);
    try writer.writeAll("defer nested.deinit(allocator);\n");
    if (typedSingularMessageField(ctx.file, field)) |_| {
        try indent(writer, depth);
    ...
    if (typedOneofMessageField(ctx.file, field)) |_| {
...
fn writeOneofValueEncode(file: *const schema.FileDescriptor, field: *const schema.FieldDescriptor, value_expr: []const u8, writer: *std.Io.Writer) Error!void {
    switch (field.kind) {
        ...
        .message => {
            if (typedOneofMessageField(file, field)) |_| {
...
fn writeFieldDecl(file: *const schema.FileDescriptor, field: *const schema.FieldDescriptor, writer: *std.Io.Writer, depth: usize) Error!void {
    ...
    try writer.writeAll(": ");
    if (typedSingularMessageField(file, field)) |type_name| {
        try writeTypedMessageType(type_name, writer);
...
fn writeMapEntryHelpers(field: *const schema.FieldDescriptor, writer: *std.Io.Writer, depth: usize) Error!void {
    const map_type = switch (field.kind) {
        .map => |map| map,
        else => return,
    };
...
fn writeEncodedSize(file: *const schema.FileDescriptor, message: *const schema.MessageDescriptor, writer: *std.Io.Writer, depth: usize) Error!void {
    ...
    for (message.oneofs.items) |oneof| try writeEncodedSizeOneof(file, message, oneof, writer, depth + 1);
    try indent(writer, depth + 1);
...
fn writeEncodedSizeField(file: *const schema.FileDescriptor, field: *const schema.FieldDescriptor, receiver: []const u8, writer: *std.Io.Writer, depth: usize) Error!void {
    ...
    try indent(writer, depth);
    if (typedSingularMessageField(file, field)) |_| {
        try writer.writeAll("if (");
...
fn writeEncodedSizeOneof(file: *const schema.FileDescriptor, message: *const schema.MessageDescriptor, oneof: schema.OneofDescriptor, writer: *std.Io.Writer, depth: usize) Error!void {
    try indent(writer, depth);
    try writer.writeAll("switch (self.");
    try writeQuotedIdent(oneof.name, writer);
    try writer.writeAll(") {\n");
...
fn writeMergeFrom(file: *const schema.FileDescriptor, message: *const schema.MessageDescriptor, writer: *std.Io.Writer, depth: usize) Error!void {
    ...
    for (message.oneofs.items) |oneof| try writeMergeOneof(file, message, oneof, writer, depth + 1);
    try indent(writer, depth + 1);
...
fn writeMergeField(file: *const schema.FileDescriptor, field: *const schema.FieldDescriptor, writer: *std.Io.Writer, depth: usize) Error!void {
    if (field.kind == .map) return try writeMergeMapField(field, writer, depth);
    if (field.cardinality == .repeated) return try writeMergeRepeatedField(file, field, writer, depth);
    switch (field.kind) {
        .message, .group => try writeMergeSingularMessageField(file, field, writer, depth),
...
fn writeMergeMapField(field: *const schema.FieldDescriptor, writer: *std.Io.Writer, depth: usize) Error!void {
    try indent(writer, depth);
    try writer.writeAll("if (other.");
    try writeQuotedIdent(field.name, writer);
    try writer.writeAll(".len != 0) {\n");
...
fn writeMergeSingularMessageField(file: *const schema.FileDescriptor, field: *const schema.FieldDescriptor, writer: *std.Io.Writer, depth: usize) Error!void {
    if (typedSingularMessageField(file, field)) |_| {
        try indent(writer, depth);
        try writer.writeAll("if (other.");
        try writeQuotedIdent(field.name, writer);
...
fn writeMergeOneof(file: *const schema.FileDescriptor, message: *const schema.MessageDescriptor, oneof: schema.OneofDescriptor, writer: *std.Io.Writer, depth: usize) Error!void {
    try indent(writer, depth);
...
fn writeDeinit(file: *const schema.FileDescriptor, message: *const schema.MessageDescriptor, writer: *std.Io.Writer, depth: usize) Error!void {
    try indent(writer, depth);
    ...
    for (message.fields.items) |*field| {
        if (typedSingularMessageField(file, field)) |_| {
            ...
            try writeQuotedIdent(field.name, writer);
            try writer.writeAll(");\n");
        } else if (field.cardinality == .repeated or field.kind == .map) {
            try indent(writer, depth + 1);
            try writer.writeAll("allocator.free(self.");
            try writeQuotedIdent(field.name, writer);
            try writer.writeAll(");\n");
    ...
    for (message.oneofs.items) |oneof| {
        ...
        for (message.fields.items) |*field| {
            if (field.oneof_name) |name| {
                if (std.mem.eql(u8, name, oneof.name) and typedOneofMessageField(file, field) != null) {
                    if (!wrote_switch) {
                        try indent(writer, depth + 1);
                        try writer.writeAll("switch (self.");
                        try writeQuotedIdent(oneof.name, writer);
...
fn writeCloneOwned(file: *const schema.FileDescriptor, message: *const schema.MessageDescriptor, writer: *std.Io.Writer, depth: usize) Error!void {
    ...
    for (message.oneofs.items) |oneof| try writeCloneOneof(file, message, oneof, writer, depth + 1);
...
fn writeCloneOneof(file: *const schema.FileDescriptor, message: *const schema.MessageDescriptor, oneof: schema.OneofDescriptor, writer: *std.Io.Writer, depth: usize) Error!void {
    try indent(writer, depth);
    try writer.writeAll("out.");
    try writeQuotedIdent(oneof.name, writer);
    try writer.writeAll(" = switch (self.");
...
fn writeDecodeMessageField(file: *const schema.FileDescriptor, field: *const schema.FieldDescriptor, writer: *std.Io.Writer, depth: usize) Error!void {
    ...
    } else if (typedSingularMessageField(file, field)) |type_name| {
        try writer.writeAll("{ const payload = ");
        try writeMessagePayloadRead(file, field, "r", writer);
        try writer.writeAll("; var nested = try ");
        try writeMessageTypeReference(type_name, writer);
...
test "codegen with registry emits extension type refs" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, content, "var nested = try imports.@\"common.proto\".@\"demo\".@\"common\".@\"Note\".parseTextWithOptions(allocator, block, .{ .ignore_unknown_fields = options.ignore_unknown_fields });") != null);
...
test "codegen emits map JSON stringify and parse helpers" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, content, "var nested = try @\"Child\".decode(allocator, entry.value); defer nested.deinit(allocator); try nested.jsonStringifyWithOptions(allocator, writer, .{ .enum_as_name = options.enum_as_name, .preserve_proto_field_names = options.preserve_proto_field_names, .always_print_primitive_fields = options.always_print_primitive_fields });") != null);
...
test "codegen emits service metadata without RPC runtime stubs" {
    const allocator = std.testing.allocator;
    var file = try @import("parser.zig").Parser.parse(allocator,
        \\syntax = "proto2";
        \\message Req {}
    ...
    try std.testing.expect(std.mem.indexOf(u8, content, "dispatchRaw") == null);
...
test "codegen validates closed enum map values in wire decode" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, content, "try @\"_unknown_fields_list\".append(allocator, raw); } else try @This().@\"appendOrReplaceMapEntry_keyed\"(allocator, &@\"keyed_list\", entry);") != null);
}
