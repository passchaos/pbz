...
pub const WellKnownType = schema.WellKnownType;
...
pub const eqlCamelcaseName = schema.eqlCamelcaseName;
pub const writeLowercaseName = schema.writeLowercaseName;
pub const writeCamelcaseName = schema.writeCamelcaseName;
pub const wellKnownTypeFromFullName = schema.wellKnownTypeFromFullName;
pub const isWellKnownTypeFullName = schema.isWellKnownTypeFullName;
...
test "root exports descriptor schema support types" {
    ...
    try std.testing.expect(eqlCamelcaseName("foo_bar", "fooBar"));
    const lower_name = try lowercaseNameAlloc(std.testing.allocator, "FooBar");
    defer std.testing.allocator.free(lower_name);
    try std.testing.expectEqualStrings("foobar", lower_name);
    try std.testing.expect(eqlLowercaseName("FooBar", "foobar"));
    ...
    try std.testing.expect(isWellKnownTypeFullName("google.protobuf.Timestamp"));
