...
pub const proto_package = "demo.messageset";
...
pub const demo = struct {
    pub const messageset = struct {
        pub const Host = struct {
            ...
            pub fn hasExtension_note(self: @This()) !bool {
                return try extensions.note.hasInUnknown(self);
            }

            pub fn countExtension_note(self: @This()) !usize {
                return try extensions.note.countInUnknown(self);
            }

            pub fn getExtension_note(self: @This(), allocator: std.mem.Allocator) !?[]const u8 {
                return try extensions.note.decodeFirstFromUnknown(self, allocator);
            }

            pub fn setExtension_note(self: *@This(), allocator: std.mem.Allocator, value: []const u8) !void {
                try extensions.note.replaceInUnknown(self, allocator, value);
            }

            pub fn setExtensionMessage_note(self: *@This(), allocator: std.mem.Allocator, value: Note) !void {
                const payload = try value.encode(allocator);
                defer allocator.free(payload);
                try extensions.note.replaceInUnknown(self, allocator, payload);
            }

            pub fn getExtensionMessage_note(self: @This(), allocator: std.mem.Allocator) !?Note {
                const payload = (try extensions.note.decodeFirstFromUnknown(self, allocator)) orelse return null;
                return try Note.decode(allocator, payload);
            }

            pub fn clearExtension_note(self: *@This(), allocator: std.mem.Allocator) !void {
                try extensions.note.clearFromUnknown(self, allocator);
            }
            ...
            pub fn jsonStringifyWithOptions(self: @This(), allocator: std.mem.Allocator, writer: *std.Io.Writer, options: @This().JsonStringifyOptions) !void {
                ...
                var first = true;
                {
                    const values = try extensions.note.decodeAllFromUnknown(self, allocator);
            ...
            fn jsonFillFromValue(self: *@This(), allocator: std.mem.Allocator, arena_allocator: std.mem.Allocator, json_value: std.json.Value, options: @This().JsonParseOptions) !void {
                ...
                while (it.next()) |entry| {
                    ...
                    if (value == .null) {
                        if (std.mem.eql(u8, key, "[demo.messageset.note]") or std.mem.eql(u8, key, "[note]")) { try self.clearUnknownFieldsByNumber(allocator, extensions.note.number); continue; }
    ...
    try std.json.Stringify.value(value, .{}, writer);
        ...
        pub const Note = struct {
            ...
            pub fn cloneOwned(self: @This(), allocator: std.mem.Allocator) !@This() {
                ...
                out.text = try owned_allocator.dupe(u8, self.text);
                out.has_text = self.has_text;
                out._unknown_fields = try pbz.wire.cloneRawFields(allocator, self._unknown_fields);
                return out;
            }
            ...
            pub fn textFieldView(bytes: []const u8) !?[]const u8 {
                return try pbz.wire.bytesFieldView(bytes, 2);
            }

            pub fn textFieldSlices(header: *[20]u8, value: []const u8) !pbz.wire.BorrowedFieldSlices {
                return pbz.wire.lengthDelimitedFieldSlicesAssumeValid(header, 2, value);
            }

            pub fn textStringView(bytes: []const u8) !?[]const u8 {
                return try textFieldView(bytes);
            }

            pub fn textStringSlices(header: *[20]u8, value: []const u8) !pbz.wire.BorrowedFieldSlices {
                return try textFieldSlices(header, value);
            }
            ...
            pub fn mergeFrom(self: *@This(), allocator: std.mem.Allocator, other: @This()) !void {
                ...
                if (other.has_text) { self.text = other.text; self.has_text = true; }
                try pbz.wire.appendRawFieldsClone(allocator, &self._unknown_fields, other._unknown_fields);
            }
            ...
            pub fn encodedSize(self: @This()) usize {
                ...
                if (self.has_text) size += 1 + pbz.wire.encodedVarintSize(self.text.len) + self.text.len;
                for (self._unknown_fields) |raw| size += raw.len;
                return size;
            }
            ...
            pub fn writeTo(self: @This(), w: *pbz.Writer) !void {
                ...
                if (self.has_text) try w.writeString(2, self.text);
                for (self._unknown_fields) |raw| try w.appendSlice(raw);
            }

            pub fn writeToAssumeCapacity(self: @This(), w: *pbz.Writer) !void {
                if (self.has_id) w.writeInt32AssumeCapacity(1, self.id);
                if (self.has_text) w.writeStringAssumeCapacity(2, self.text);
                for (self._unknown_fields) |raw| w.appendSliceAssumeCapacity(raw);
            }
            ...
            pub fn encodeIntoAssumeCapacity(self: @This(), buffer: []u8) ![]u8 {
                ...
                if (self.has_text) { buffer[index] = 18; index += 1; pbz.wire.writeVarintToSlice(buffer, &index, self.text.len); @memcpy(buffer[index..][0..self.text.len], self.text); index += self.text.len; }
            ...
            pub fn decodeFromReader(allocator: std.mem.Allocator, r: *pbz.Reader) !@This() {
                ...
                while (!r.eof()) {
                    ...
                    switch (raw_tag) {
                        ...
                        18 => { self.text = try r.readBytes(); self.has_text = true; },
            ...
            pub fn jsonStringifyWithOptions(self: @This(), allocator: std.mem.Allocator, writer: *std.Io.Writer, options: @This().JsonStringifyOptions) !void {
                ...
                if (self.has_text or options.always_print_primitive_fields) {
                    try writer.writeAll(if (first) "\"text\":" else ",\"text\":"); first = false;
            ...
            fn jsonFillFromValue(self: *@This(), allocator: std.mem.Allocator, arena_allocator: std.mem.Allocator, json_value: std.json.Value, options: @This().JsonParseOptions) !void {
                ...
                while (it.next()) |entry| {
                    ...
                    if (std.mem.eql(u8, key, "text")) {
                        self.text = try @This().jsonString(value);
                        self.has_text = true;
                        continue;
                    }
                    if (options.ignore_unknown_fields) continue;
...
fn jsonWriteString(writer: *std.Io.Writer, value: []const u8) !void {
    ...
    try std.json.Stringify.value(value, .{}, writer);
            ...
            pub fn formatTextWithOptions(self: @This(), allocator: std.mem.Allocator, writer: *std.Io.Writer, options: @This().TextFormatOptions) !void {
                ...
                if (self.has_text) { try writer.writeAll("text: "); const value = self.text; try @This().textWriteQuotedBytes(value, writer); try writer.writeByte('\n'); }
                for (self._unknown_fields) |raw| {
                    try @This().textWriteUnknownRaw(raw, writer);
                }
            }
            ...
            pub fn parseTextWithOptions(allocator: std.mem.Allocator, text: []const u8, options: @This().TextParseOptions) !@This() {
                ...
                while (lines.next()) |raw_line| {
                    ...
                    if (@This().textFieldValue(line, "text")) |raw_value| {
                        self.text = try @This().textUnquote(try self._pbzOwnedAllocator(allocator), raw_value);
                        self.has_text = true;
                        continue;
                    }
                    if (try @This().textUnknownField(allocator, line)) |raw| { try pbz.wire.appendOwnedRawField(allocator, &_unknown_fields_list, raw); continue; }
        ...
        pub const extensions = struct {
            pub const note = struct {
                ...
                pub fn setMessageOn(message: *Host, allocator: std.mem.Allocator, value: Note) !void {
                ...
                pub fn getMessageOn(message: Host, allocator: std.mem.Allocator) !?Note {
                    const _pbz_payload = (try decodeFirstFromUnknown(message, allocator)) orelse return null;
                    return try Note.decode(allocator, _pbz_payload);
                }
                pub fn write(w: *pbz.Writer, value: []const u8) !void {
                    try w.writeTag(1, .start_group);
                    try w.writeUInt32(2, 100);
                    try w.writeMessage(3, value);
                    try w.writeTag(1, .end_group);
                }
                ...
                pub fn hasInUnknown(message: anytype) !bool {
                    for (message.unknownFields()) |_pbz_raw| { if ((try decodeRaw(_pbz_raw)) != null) return true; }
                    return false;
                }
                pub fn countInUnknown(message: anytype) !usize {
                    var _pbz_count: usize = 0;
                    for (message.unknownFields()) |_pbz_raw| { if ((try decodeRaw(_pbz_raw)) != null) _pbz_count += 1; }
                    return _pbz_count;
                }
                pub fn clearFromUnknown(message: anytype, allocator: std.mem.Allocator) !void {
                    const _pbz_old = message.unknownFields();
                    if (_pbz_old.len == 0) return;
                    var _pbz_keep: std.ArrayList([]const u8) = .empty;
                    errdefer _pbz_keep.deinit(allocator);
                    for (_pbz_old) |_pbz_raw| { if ((try decodeRaw(_pbz_raw)) == null) try _pbz_keep.append(allocator, _pbz_raw) else allocator.free(_pbz_raw); }
                    if (_pbz_keep.items.len == 0) { _pbz_keep.deinit(allocator); message._unknown_fields = &.{}; } else message._unknown_fields = try _pbz_keep.toOwnedSlice(allocator);
                    allocator.free(_pbz_old);
                }
                ...
                pub fn decodeRaw(raw: []const u8) !?[]const u8 {
                    ...
                    if (_pbz_tag.number == 1 and _pbz_tag.wire_type == .start_group) { const _pbz_value = try decodeMessageSetItem(&r); if (!r.eof()) return error.InvalidWireType; return _pbz_value; }
                    if (_pbz_tag.number == 100 and _pbz_tag.wire_type == .length_delimited) { const _pbz_value = try r.readBytes(); if (!r.eof()) return error.InvalidWireType; return _pbz_value; }
                    return null;
                }
                ...
                pub fn decodeMessageSetItem(r: *pbz.Reader) !?[]const u8 {
                    var type_id: ?u32 = null;
                    var _pbz_payload: ?[]const u8 = null;
                    while (try r.nextTag()) |_pbz_tag| {
                        if (_pbz_tag.wire_type == .end_group) {
                            if (_pbz_tag.number != 1) return error.InvalidFieldNumber;
                            return if (type_id != null and type_id.? == 100) _pbz_payload else null;
                        }
                        switch (_pbz_tag.number) {
                            2 => { if (_pbz_tag.wire_type != .varint) return error.InvalidWireType; const raw_type_id = try r.readUInt32(); if (raw_type_id == 0 or raw_type_id > std.math.maxInt(pbz.FieldNumber)) return error.InvalidFieldNumber; type_id = raw_type_id; },
                            3 => { if (_pbz_tag.wire_type != .length_delimited) return error.InvalidWireType; _pbz_payload = try r.readBytes(); },
                            else => try r.skipValue(_pbz_tag),
                        }
                    }
                    return error.TruncatedInput;
                }
