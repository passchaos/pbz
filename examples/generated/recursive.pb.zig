...
pub const proto_package = "demo.recursive";
pub const proto_syntax = "proto3";

pub const demo = struct {
    pub const recursive = struct {
        pub const Node = struct {
            pub const child_number = 1;
            pub const children_number = 2;
            ...
            pub const children_field = struct {
                pub const number = 2;
            ...
            pub fn deinit(self: *@This(), allocator: std.mem.Allocator) void {
                for (self.children) |value| { var mutable = value; mutable.deinit(allocator); }
            ...
            pub fn cloneOwned(self: @This(), allocator: std.mem.Allocator) !@This() {
                ...
                out.child = try owned_allocator.dupe(u8, self.child);
                out.has_child = self.has_child;
                if (self.children.len != 0) {
                    const cloned = try allocator.alloc(Node, self.children.len);
                    for (self.children, 0..) |item, i| cloned[i] = try item.cloneOwned(allocator);
                    out.children = cloned;
                }
            ...
            pub fn mergeFrom(self: *@This(), allocator: std.mem.Allocator, other: @This()) !void {
                if (other.has_child) {
                    if (self.has_child and self.child.len != 0 and other.child.len != 0) {
                        const owned_allocator = try self._pbzOwnedAllocator(allocator);
                        const merged = try owned_allocator.alloc(u8, self.child.len + other.child.len);
                        @memcpy(merged[0..self.child.len], self.child);
                        @memcpy(merged[self.child.len..], other.child);
                        self.child = merged;
                    } else if (!self.has_child or self.child.len == 0) {
                ...
                if (other.children.len != 0) {
                    ...
                    const merged = try allocator.alloc(Node, old.len + other.children.len);
                    @memcpy(merged[0..old.len], old);
                    for (other.children, 0..) |item, i| merged[old.len + i] = try item.cloneOwned(allocator);
            ...
            pub fn encodedSize(self: @This()) usize {
                var size: usize = 0;
                if (self.has_child) size += 1 + pbz.wire.encodedVarintSize(self.child.len) + self.child.len;
                for (self.children) |item| { const payload_len = item.encodedSize(); size += 1 + pbz.wire.encodedVarintSize(payload_len) + payload_len; }
                for (self._unknown_fields) |raw| size += raw.len;
                return size;
            }

            pub fn writeTo(self: @This(), w: *pbz.Writer) !void {
                if (self.has_child) try w.writeMessage(1, self.child);
                for (self.children) |item| { const payload_len = item.encodedSize(); try w.writeTag(2, .length_delimited); try w.writeVarint(payload_len); try item.writeTo(w); }
                for (self._unknown_fields) |raw| try w.appendSlice(raw);
            }

            pub fn writeToAssumeCapacity(self: @This(), w: *pbz.Writer) !void {
                                if (self.has_child) try w.writeMessage(1, self.child);
                for (self.children) |item| { const payload_len = item.encodedSize(); w.writeTagAssumeCapacity(2, .length_delimited); w.writeVarintAssumeCapacity(payload_len); try item.writeToAssumeCapacity(w); }
            ...
            pub fn decodeFromReader(allocator: std.mem.Allocator, r: *pbz.Reader) !@This() {
                ...
                var children_list: std.ArrayList(Node) = .empty;
                defer children_list.deinit(allocator);
                errdefer for (children_list.items) |item| { var mutable = item; mutable.deinit(allocator); };
            ...
            pub fn decodeReuse(self: *@This(), allocator: std.mem.Allocator, bytes: []const u8) !void {
                var children_list: std.ArrayList(Node) = std.ArrayList(Node).fromOwnedSlice(@constCast(self.children));
                for (children_list.items) |*item| item.deinit(allocator);
                children_list.clearRetainingCapacity();
                self.children = &.{};
                ...
                while (lines.next()) |raw_line| {
                    ...
                    if (@This().textBlockField(line, "child")) {
                        ...
                        if (self.has_child and self.child.len != 0 and payload.len != 0) {
                            ...
                            @memcpy(merged[0..self.child.len], self.child);
                            @memcpy(merged[self.child.len..], payload);
                            self.child = merged;
                        } else {
                            self.child = payload;
                        }
