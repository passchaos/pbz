...
pub const proto_package = "demo.closedenum";
pub const proto_syntax = "proto2";

pub const demo = struct {
    pub const closedenum = struct {
        pub const Kind = enum(i32) {
            ...
            pub fn fromName(name: []const u8) ?@This() {
                ...
                if (std.mem.eql(u8, name, "KIND_OK")) return .KIND_OK;
                return null;
            }
        ...
        pub const ClosedEnum = struct {
            ...
            pub const kind_field = struct {
                ...
                pub const kind = "enum";
                pub const type_name = "demo.closedenum.Kind";
            ...
            pub fn mergeFrom(self: *@This(), allocator: std.mem.Allocator, other: @This()) !void {
                if (other.has_kind) { self.kind = other.kind; self.has_kind = true; }
            ...
            pub fn encodedSize(self: @This()) usize {
                var size: usize = 0;
                if (self.has_kind) size += 1 + pbz.wire.encodedVarintSize(@as(u64, @bitCast(@as(i64, self.kind))));
                for (self.history) |item| size += 1 + pbz.wire.encodedVarintSize(@as(u64, @bitCast(@as(i64, item))));
                for (self._unknown_fields) |raw| size += raw.len;
                return size;
            }

            pub fn writeTo(self: @This(), w: *pbz.Writer) !void {
                if (self.has_kind) try w.writeInt32(1, self.kind);
                for (self.history) |item| try w.writeInt32(2, item);
                for (self._unknown_fields) |raw| try w.appendSlice(raw);
            }

            pub fn writeToAssumeCapacity(self: @This(), w: *pbz.Writer) !void {
                if (self.has_kind) w.writeInt32AssumeCapacity(1, self.kind);
                for (self.history) |item| w.writeInt32AssumeCapacity(2, item);
            ...
            pub fn encodeIntoAssumeCapacity(self: @This(), buffer: []u8) ![]u8 {
                var index: usize = 0;
                if (self.has_kind) { buffer[index] = 8; index += 1; if (self.kind > 0 and self.kind < 0x80) { buffer[index] = @intCast(self.kind); index += 1; } else pbz.wire.writeVarintToSlice(buffer, &index, @as(u64, @bitCast(@as(i64, self.kind)))); }
                for (self.history) |item| { buffer[index] = 16; index += 1; pbz.wire.writeVarintToSlice(buffer, &index, @as(u64, @bitCast(@as(i64, item)))); }
            ...
            pub fn decodeFromReader(allocator: std.mem.Allocator, r: *pbz.Reader) !@This() {
                ...
                while (try r.nextTag()) |tag| {
                    switch (tag.number) {
                        1 => { const value = try r.readInt32(); if (!@This().enumKnown(value, &.{0, 1})) { try pbz.wire.appendConsumedRawField(allocator, &_unknown_fields_list, r, r.lastTagStart()); } else { self.kind = value; self.has_kind = true; } },
                        2 => {
                            if (tag.wire_type == .length_delimited) {
                                const payload = try r.readBytes();
                                var packed_reader = pbz.Reader.init(payload);
                                while (!packed_reader.eof()) { const value_start = packed_reader.position(); const value = try packed_reader.readInt32(); const value_end = packed_reader.position(); if (!@This().enumKnown(value, &.{0, 1})) { try pbz.wire.appendRawVarintPayload(allocator, &_unknown_fields_list, 2, payload[value_start..value_end]); continue; } try history_list.append(allocator, value); }
                            } else {
                                { const value = try r.readInt32(); if (!@This().enumKnown(value, &.{0, 1})) { try pbz.wire.appendConsumedRawField(allocator, &_unknown_fields_list, r, r.lastTagStart()); } else { try history_list.append(allocator, value); } }
                            }
                        },
            ...
            pub fn jsonStringifyWithOptions(self: @This(), allocator: std.mem.Allocator, writer: *std.Io.Writer, options: @This().JsonStringifyOptions) !void {
                ...
                try writer.writeAll("{");
                var first = true;
                if (self.has_kind or options.always_print_primitive_fields) {
                    try writer.writeAll(if (first) "\"kind\":" else ",\"kind\":"); first = false;
                    const value = self.kind;
                    try @This().jsonWriteEnum(writer, value, &.{"KIND_UNKNOWN", "KIND_OK"}, &.{0, 1}, options.enum_as_name);
                }
                if (self.history.len != 0 or options.always_print_primitive_fields) {
                    ...
                    try writer.writeAll("[");
                    for (self.history, 0..) |item, i| { if (i != 0) try writer.writeAll(","); try @This().jsonWriteEnum(writer, item, &.{"KIND_UNKNOWN", "KIND_OK"}, &.{0, 1}, options.enum_as_name); }
                    try writer.writeAll("]");
                }
                try writer.writeAll("}");
            }
            ...
            fn jsonFillFromValue(self: *@This(), allocator: std.mem.Allocator, arena_allocator: std.mem.Allocator, json_value: std.json.Value, options: @This().JsonParseOptions) !void {
                ...
                while (it.next()) |entry| {
                    ...
                    if (std.mem.eql(u8, key, "kind")) {
                        self.kind = @This().jsonEnum(value, &.{"KIND_UNKNOWN", "KIND_OK"}, &.{0, 1}, true) catch |err| { if (options.ignore_unknown_fields) continue; return err; };
                        self.has_kind = true;
                        continue;
                    }
                    if (std.mem.eql(u8, key, "history")) {
                        ...
                        for (array.items) |item| {
                            const parsed_enum = @This().jsonEnum(item, &.{"KIND_UNKNOWN", "KIND_OK"}, &.{0, 1}, true) catch |err| { if (options.ignore_unknown_fields) continue; return err; };
...
fn jsonWriteString(writer: *std.Io.Writer, value: []const u8) !void {
    ...
    try std.json.Stringify.value(value, .{}, writer);
            ...
            pub fn formatTextWithOptions(self: @This(), allocator: std.mem.Allocator, writer: *std.Io.Writer, options: @This().TextFormatOptions) !void {
                _ = allocator;
                if (self.has_kind) { try writer.writeAll("kind: "); const value = self.kind; try @This().textWriteEnum(writer, value, &.{"KIND_UNKNOWN", "KIND_OK"}, &.{0, 1}, options.enum_as_name); try writer.writeByte('\n'); }
                for (self.history) |value| { try writer.writeAll("history: "); try @This().textWriteEnum(writer, value, &.{"KIND_UNKNOWN", "KIND_OK"}, &.{0, 1}, options.enum_as_name); try writer.writeByte('\n'); }
            ...
            pub fn parseTextWithOptions(allocator: std.mem.Allocator, text: []const u8, options: @This().TextParseOptions) !@This() {
                ...
                while (lines.next()) |raw_line| {
                    ...
                    if (@This().textFieldValue(line, "kind")) |raw_value| {
                        self.kind = @This().textEnum(raw_value, &.{"KIND_UNKNOWN", "KIND_OK"}, &.{0, 1}, true) catch |err| { if (options.ignore_unknown_fields) { continue; } return err; };
                        self.has_kind = true;
                        continue;
                    }
                    if (@This().textFieldValue(line, "history")) |raw_value| {
                        const parsed_enum = @This().textEnum(raw_value, &.{"KIND_UNKNOWN", "KIND_OK"}, &.{0, 1}, true) catch |err| { if (options.ignore_unknown_fields) { continue; } return err; };
