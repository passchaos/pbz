...
pub const proto_package = "demo.publics.mid";
pub const proto_syntax = "proto3";
pub const imports = struct {
    pub const public_leaf_proto = @import("public_leaf.pb.zig");
    pub const public_leaf_proto_path = "public_leaf.proto";
    pub const public_leaf_proto_kind = "public";
};
...
pub const demo = struct {
    pub const publics = struct {
        pub const mid = struct {
            pub const Mid = struct {
                pub const leaf_number = 1;

                pub const leaf_field = struct {
                    pub const number = 1;
                    pub const name = "leaf";
                    pub const json_name = "leaf";
                    pub const cardinality = "optional";
                    pub const kind = "message";
                    pub const type_name = "demo.publics.leaf.Leaf";
                    pub const zig_type = "?imports.public_leaf_proto.demo.publics.leaf.Leaf";
                    pub const has_type_ref = true;
                    pub const type_ref = imports.public_leaf_proto.demo.publics.leaf.Leaf;
                ...
                leaf: ?imports.public_leaf_proto.demo.publics.leaf.Leaf = null,
                ...
                pub fn deinit(self: *@This(), allocator: std.mem.Allocator) void {
                    if (self.leaf) |*value| value.deinit(allocator);
                ...
                pub fn cloneOwned(self: @This(), allocator: std.mem.Allocator) !@This() {
                    ...
                    if (self.leaf) |value| out.leaf = try value.cloneOwned(allocator);
                ...
                pub fn mergeFrom(self: *@This(), allocator: std.mem.Allocator, other: @This()) !void {
                    if (other.leaf) |other_value| {
                        if (self.leaf) |*self_value| { try self_value.mergeFrom(allocator, other_value); } else { self.leaf = try other_value.cloneOwned(allocator); }
                    }
                ...
                pub fn encodedSize(self: @This()) usize {
                    var size: usize = 0;
                    if (self.leaf) |value| { const payload_len = value.encodedSize(); size += 1 + pbz.wire.encodedVarintSize(payload_len) + payload_len; }
                    for (self._unknown_fields) |raw| size += raw.len;
                    return size;
                }

                pub fn writeTo(self: @This(), w: *pbz.Writer) !void {
                    if (self.leaf) |value| { const payload_len = value.encodedSize(); try w.writeTag(1, .length_delimited); try w.writeVarint(payload_len); try value.writeTo(w); }
                    for (self._unknown_fields) |raw| try w.appendSlice(raw);
                }

                pub fn writeToAssumeCapacity(self: @This(), w: *pbz.Writer) !void {
                    if (self.leaf) |value| { const payload_len = value.encodedSize(); w.writeTagAssumeCapacity(1, .length_delimited); w.writeVarintAssumeCapacity(payload_len); try value.writeToAssumeCapacity(w); }
                ...
                pub fn encodeIntoAssumeCapacity(self: @This(), buffer: []u8) ![]u8 {
                    var index: usize = 0;
                    if (self.leaf) |value| { const payload_len = value.encodedSize(); buffer[index] = 10; index += 1; pbz.wire.writeVarintToSlice(buffer, &index, payload_len); _ = try value.encodeIntoAssumeCapacity(buffer[index..][0..payload_len]); index += payload_len; }
                ...
                pub fn encodeIntoAssumeCapacityTrustedUtf8(self: @This(), buffer: []u8) ![]u8 {
                    var index: usize = 0;
                    if (self.leaf) |value| { const payload_len = value.encodedSize(); buffer[index] = 10; index += 1; pbz.wire.writeVarintToSlice(buffer, &index, payload_len); _ = try value.encodeIntoAssumeCapacityTrustedUtf8(buffer[index..][0..payload_len]); index += payload_len; }
                    for (self._unknown_fields) |raw| { @memcpy(buffer[index..][0..raw.len], raw); index += raw.len; }
                    return buffer[0..index];
                }

                pub fn writeDeterministicTo(self: @This(), allocator: std.mem.Allocator, w: *pbz.Writer) !void {
                    if (self.leaf) |item| { const payload_len = item.encodedSize(); try w.writeTag(1, .length_delimited); try w.writeVarint(payload_len); try item.writeDeterministicTo(allocator, w); }
                ...
                pub fn writeDeterministicToAssumeCapacity(self: @This(), allocator: std.mem.Allocator, w: *pbz.Writer) !void {
                    if (self.leaf) |item| { const payload_len = item.encodedSize(); w.writeTagAssumeCapacity(1, .length_delimited); w.writeVarintAssumeCapacity(payload_len); try item.writeDeterministicToAssumeCapacity(allocator, w); }
                ...
                pub fn decodeFromReader(allocator: std.mem.Allocator, r: *pbz.Reader) !@This() {
                    ...
                    while (try r.nextTag()) |tag| {
                        switch (tag.number) {
                            1 => { const payload = try r.readBytes(); var payload_reader = try r.nested(payload); var _pbz_nested = try imports.public_leaf_proto.demo.publics.leaf.Leaf.decodeFromReader(allocator, &payload_reader); errdefer _pbz_nested.deinit(allocator); if (self.leaf) |*existing| { try existing.mergeFrom(allocator, _pbz_nested); _pbz_nested.deinit(allocator); } else { self.leaf = _pbz_nested; } },
                ...
                pub fn missingRequiredFieldPath(self: @This(), allocator: std.mem.Allocator) !?[]u8 {
                    if (self.leaf) |_pbz_nested| {
                        if (try _pbz_nested.missingRequiredFieldPath(allocator)) |suffix| { defer allocator.free(suffix); return try std.fmt.allocPrint(allocator, "leaf.{s}", .{suffix}); }
                    }
                    return null;
                }
                ...
                pub fn validateRequiredRecursive(self: @This(), allocator: std.mem.Allocator) !void {
                    ...
                    if (self.leaf) |_pbz_nested| try _pbz_nested.validateRequiredRecursive(allocator);
                }
                ...
                pub fn jsonStringifyWithOptions(self: @This(), allocator: std.mem.Allocator, writer: *std.Io.Writer, options: @This().JsonStringifyOptions) !void {
                    ...
                    var first = true;
                    if (self.leaf) |_pbz_nested| {
                        try writer.writeAll(if (first) "\"leaf\":" else ",\"leaf\":"); first = false;
                    ...
                    while (it.next()) |entry| {
                        ...
                        if (value == .null) {
                            if (std.mem.eql(u8, key, "leaf")) {
                                if (self.leaf) |*old_value| old_value.deinit(allocator); self.leaf = null;
                                continue;
                            }
                        ...
                        if (std.mem.eql(u8, key, "leaf")) {
                            var _pbz_nested = try imports.public_leaf_proto.demo.publics.leaf.Leaf.jsonParseValueWithOptions(allocator, arena_allocator, value, .{ .ignore_unknown_fields = options.ignore_unknown_fields });
                            errdefer _pbz_nested.deinit(allocator);
                            if (self.leaf) |*existing| { try existing.mergeFrom(allocator, _pbz_nested); _pbz_nested.deinit(allocator); } else { self.leaf = _pbz_nested; }
                            continue;
                        }
                        if (options.ignore_unknown_fields) continue;
                        return error.UnknownField;
    ...
    try std.json.Stringify.value(value, .{}, writer);
                ...
                pub fn parseTextWithOptions(allocator: std.mem.Allocator, text: []const u8, options: @This().TextParseOptions) !@This() {
                    ...
                    while (lines.next()) |raw_line| {
                        const line = @This().textCleanLine(raw_line, text_has_comments);
                        if (line.len == 0) continue;
                        if (@This().textBlockField(line, "leaf")) {
                            const block = try @This().textBlock(allocator, &lines, text_has_comments);
                            defer allocator.free(block);
                            var _pbz_nested = try imports.public_leaf_proto.demo.publics.leaf.Leaf.parseTextWithOptions(allocator, block, .{ .ignore_unknown_fields = options.ignore_unknown_fields });
                            if (self.leaf) |*existing| { defer _pbz_nested.deinit(allocator); try existing.mergeFrom(allocator, _pbz_nested); } else { errdefer _pbz_nested.deinit(allocator); self.leaf = _pbz_nested; }
                            continue;
                        }
