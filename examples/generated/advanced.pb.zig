    ...
    pub const advanced = struct {
        ...
        pub const Envelope = struct {
            ...
            pub fn encodeDeterministic(self: @This(), allocator: std.mem.Allocator) ![]u8 {
                ...
                if (self.kind != 0) try w.writeInt32(2, self.kind);
                if (self.has_audit) {
                    const item = self.audit;
                    var nested = try Audit.decode(allocator, item);
                    defer nested.deinit(allocator);
                    const payload = try nested.encodeDeterministic(allocator);
                    defer allocator.free(payload);
                    try w.writeMessage(3, payload);
                }
                switch (self.subject) {
                    .user_name => |value| {
                        if (!std.unicode.utf8ValidateSlice(value)) return error.InvalidUtf8;
            ...
            fn jsonFillFromValue(self: *@This(), allocator: std.mem.Allocator, arena_allocator: std.mem.Allocator, json_value: std.json.Value, options: @This().JsonParseOptions) !void {
                ...
                while (it.next()) |entry| {
                    ...
                    if (std.mem.eql(u8, key, "kind") or std.mem.eql(u8, key, "kind")) {
                        self.kind = @This().jsonEnum(value, &.{ "KIND_UNKNOWN", "KIND_PERSON", "KIND_ORG" }, &.{ 0, 1, 2 }, false) catch |err| {
                            if (options.ignore_unknown_fields) continue;
                            return err;
                        };
                        continue;
            ...
            pub fn formatTextWithOptions(self: @This(), allocator: std.mem.Allocator, writer: *std.Io.Writer, options: @This().TextFormatOptions) !void {
                ...
                switch (self.subject) {
                    ...
                    .organization_id => |value| {
                        try writer.writeAll("organization_id: ");
                        try @This().textWriteQuotedBytes(value, writer);
                        try writer.writeByte('\n');
                    },
                    .audit_subject => |value| {
            ...
            pub fn parseTextWithOptions(allocator: std.mem.Allocator, text: []const u8, options: @This().TextParseOptions) !@This() {
                ...
                while (lines.next()) |raw_line| {
                    ...
                    if (@This().textFieldValue(line, "kind")) |raw_value| {
                        self.kind = @This().textEnum(raw_value, &.{ "KIND_UNKNOWN", "KIND_PERSON", "KIND_ORG" }, &.{ 0, 1, 2 }, false) catch |err| {
            ...
            pub const Audit = struct {
                ...
                pub fn writeTo(self: @This(), w: *pbz.Writer) !void {
                    if (self.actor.len != 0) {
                        if (!std.unicode.utf8ValidateSlice(self.actor)) return error.InvalidUtf8;
                        try w.writeString(1, self.actor);
                    }
                ...
                pub fn writeToAssumeCapacity(self: @This(), w: *pbz.Writer) !void {
                    if (self.actor.len != 0) {
                        if (!std.unicode.utf8ValidateSlice(self.actor)) return error.InvalidUtf8;
                        w.writeStringAssumeCapacity(1, self.actor);
                    }
        ...
        pub const services = struct {
            pub const Directory = struct {
                ...
                pub const Get = struct {
                    pub const name = "Get";
                    pub const input_type = "Envelope";
                    pub const output_type = "Envelope";
