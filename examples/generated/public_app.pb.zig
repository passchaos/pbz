...
pub const proto_package = "demo.publics.app";
pub const proto_syntax = "proto3";
pub const imports = struct {
    pub const public_mid_proto = @import("public_mid.pb.zig");
    pub const public_mid_proto_path = "public_mid.proto";
    pub const public_mid_proto_kind = "normal";
};
const pbz_generated_file = @This();

pub const demo = struct {
    pub const publics = struct {
        pub const app = struct {
            pub const App = struct {
                pub const direct_leaf_number = 1;
                pub const mid_number = 2;

                pub const direct_leaf_field = struct {
                    pub const number = 1;
                    pub const name = "direct_leaf";
                    pub const json_name = "directLeaf";
                    pub const cardinality = "optional";
                    pub const kind = "message";
                    pub const type_name = "demo.publics.leaf.Leaf";
                    pub const zig_type = "?imports.public_mid_proto.imports.public_leaf_proto.demo.publics.leaf.Leaf";
                    pub const has_type_ref = true;
                    pub const type_ref = imports.public_mid_proto.imports.public_leaf_proto.demo.publics.leaf.Leaf;
                ...
                pub const mid_field = struct {
                    pub const number = 2;
                    pub const name = "mid";
                    pub const json_name = "mid";
                    pub const cardinality = "optional";
                    pub const kind = "message";
                    pub const type_name = "demo.publics.mid.Mid";
                    pub const zig_type = "?imports.public_mid_proto.demo.publics.mid.Mid";
                    pub const has_type_ref = true;
                    pub const type_ref = imports.public_mid_proto.demo.publics.mid.Mid;
                    pub const has_enum_ref = false;
                    pub const enum_ref = void;
                    pub const has_presence = true;
                    pub const default_value = "";
                ...
                direct_leaf: ?imports.public_mid_proto.imports.public_leaf_proto.demo.publics.leaf.Leaf = null,
                mid: ?imports.public_mid_proto.demo.publics.mid.Mid = null,
                ...
                pub fn deinit(self: *@This(), allocator: std.mem.Allocator) void {
                    if (self.direct_leaf) |*value| value.deinit(allocator);
                    if (self.mid) |*value| value.deinit(allocator);
                ...
                pub fn cloneOwned(self: @This(), allocator: std.mem.Allocator) !@This() {
                    ...
                    if (self.direct_leaf) |value| out.direct_leaf = try value.cloneOwned(allocator);
                    if (self.mid) |value| out.mid = try value.cloneOwned(allocator);
                ...
                pub fn mergeFrom(self: *@This(), allocator: std.mem.Allocator, other: @This()) !void {
                    if (other.direct_leaf) |other_value| {
                        if (self.direct_leaf) |*self_value| { try self_value.mergeFrom(allocator, other_value); } else { self.direct_leaf = try other_value.cloneOwned(allocator); }
                    }
                    if (other.mid) |other_value| {
                        if (self.mid) |*self_value| { try self_value.mergeFrom(allocator, other_value); } else { self.mid = try other_value.cloneOwned(allocator); }
                    }
                ...
                pub fn encodedSize(self: @This()) usize {
                    var size: usize = 0;
                    if (self.direct_leaf) |value| { const payload_len = value.encodedSize(); size += 1 + pbz.wire.encodedVarintSize(payload_len) + payload_len; }
                    if (self.mid) |value| { const payload_len = value.encodedSize(); size += 1 + pbz.wire.encodedVarintSize(payload_len) + payload_len; }
                    for (self._unknown_fields) |raw| size += raw.len;
                    return size;
                }

                pub fn writeTo(self: @This(), w: *pbz.Writer) !void {
                    if (self.direct_leaf) |value| { const payload_len = value.encodedSize(); try w.writeTag(1, .length_delimited); try w.writeVarint(payload_len); try value.writeTo(w); }
                    if (self.mid) |value| { const payload_len = value.encodedSize(); try w.writeTag(2, .length_delimited); try w.writeVarint(payload_len); try value.writeTo(w); }
                    for (self._unknown_fields) |raw| try w.appendSlice(raw);
                }

                pub fn writeToAssumeCapacity(self: @This(), w: *pbz.Writer) !void {
                    if (self.direct_leaf) |value| { const payload_len = value.encodedSize(); w.writeTagAssumeCapacity(1, .length_delimited); w.writeVarintAssumeCapacity(payload_len); try value.writeToAssumeCapacity(w); }
                    if (self.mid) |value| { const payload_len = value.encodedSize(); w.writeTagAssumeCapacity(2, .length_delimited); w.writeVarintAssumeCapacity(payload_len); try value.writeToAssumeCapacity(w); }
                ...
                pub fn encodeIntoAssumeCapacity(self: @This(), buffer: []u8) ![]u8 {
                    var index: usize = 0;
                    if (self.direct_leaf) |value| { const payload_len = value.encodedSize(); buffer[index] = 10; index += 1; pbz.wire.writeVarintToSlice(buffer, &index, payload_len); _ = try value.encodeIntoAssumeCapacity(buffer[index..][0..payload_len]); index += payload_len; }
                    if (self.mid) |value| { const payload_len = value.encodedSize(); buffer[index] = 18; index += 1; pbz.wire.writeVarintToSlice(buffer, &index, payload_len); _ = try value.encodeIntoAssumeCapacity(buffer[index..][0..payload_len]); index += payload_len; }
                ...
                pub fn encodeIntoAssumeCapacityTrustedUtf8(self: @This(), buffer: []u8) ![]u8 {
                    var index: usize = 0;
                    if (self.direct_leaf) |value| { const payload_len = value.encodedSize(); buffer[index] = 10; index += 1; pbz.wire.writeVarintToSlice(buffer, &index, payload_len); _ = try value.encodeIntoAssumeCapacityTrustedUtf8(buffer[index..][0..payload_len]); index += payload_len; }
                    if (self.mid) |value| { const payload_len = value.encodedSize(); buffer[index] = 18; index += 1; pbz.wire.writeVarintToSlice(buffer, &index, payload_len); _ = try value.encodeIntoAssumeCapacityTrustedUtf8(buffer[index..][0..payload_len]); index += payload_len; }
                    for (self._unknown_fields) |raw| { @memcpy(buffer[index..][0..raw.len], raw); index += raw.len; }
                    return buffer[0..index];
                }

                pub fn writeDeterministicTo(self: @This(), allocator: std.mem.Allocator, w: *pbz.Writer) !void {
                    if (self.direct_leaf) |item| { const payload_len = item.encodedSize(); try w.writeTag(1, .length_delimited); try w.writeVarint(payload_len); try item.writeDeterministicTo(allocator, w); }
                    if (self.mid) |item| { const payload_len = item.encodedSize(); try w.writeTag(2, .length_delimited); try w.writeVarint(payload_len); try item.writeDeterministicTo(allocator, w); }
                ...
                pub fn writeDeterministicToAssumeCapacity(self: @This(), allocator: std.mem.Allocator, w: *pbz.Writer) !void {
                    if (self.direct_leaf) |item| { const payload_len = item.encodedSize(); w.writeTagAssumeCapacity(1, .length_delimited); w.writeVarintAssumeCapacity(payload_len); try item.writeDeterministicToAssumeCapacity(allocator, w); }
                    if (self.mid) |item| { const payload_len = item.encodedSize(); w.writeTagAssumeCapacity(2, .length_delimited); w.writeVarintAssumeCapacity(payload_len); try item.writeDeterministicToAssumeCapacity(allocator, w); }
                ...
                pub fn decodeFromReader(allocator: std.mem.Allocator, r: *pbz.Reader) !@This() {
                    ...
                    while (try r.nextTag()) |tag| {
                        switch (tag.number) {
                            1 => { const payload = try r.readBytes(); var payload_reader = try r.nested(payload); var _pbz_nested = try imports.public_mid_proto.imports.public_leaf_proto.demo.publics.leaf.Leaf.decodeFromReader(allocator, &payload_reader); errdefer _pbz_nested.deinit(allocator); if (self.direct_leaf) |*existing| { try existing.mergeFrom(allocator, _pbz_nested); _pbz_nested.deinit(allocator); } else { self.direct_leaf = _pbz_nested; } },
                            2 => { const payload = try r.readBytes(); var payload_reader = try r.nested(payload); var _pbz_nested = try imports.public_mid_proto.demo.publics.mid.Mid.decodeFromReader(allocator, &payload_reader); errdefer _pbz_nested.deinit(allocator); if (self.mid) |*existing| { try existing.mergeFrom(allocator, _pbz_nested); _pbz_nested.deinit(allocator); } else { self.mid = _pbz_nested; } },
                ...
                pub fn missingRequiredFieldPath(self: @This(), allocator: std.mem.Allocator) !?[]u8 {
                    if (self.direct_leaf) |_pbz_nested| {
                        if (try _pbz_nested.missingRequiredFieldPath(allocator)) |suffix| { defer allocator.free(suffix); return try std.fmt.allocPrint(allocator, "direct_leaf.{s}", .{suffix}); }
                    }
                    if (self.mid) |_pbz_nested| {
                        if (try _pbz_nested.missingRequiredFieldPath(allocator)) |suffix| { defer allocator.free(suffix); return try std.fmt.allocPrint(allocator, "mid.{s}", .{suffix}); }
                    }
                    return null;
                }
                ...
                pub fn validateRequiredRecursive(self: @This(), allocator: std.mem.Allocator) !void {
                    ...
                    if (self.direct_leaf) |_pbz_nested| try _pbz_nested.validateRequiredRecursive(allocator);
                    if (self.mid) |_pbz_nested| try _pbz_nested.validateRequiredRecursive(allocator);
                }
                ...
                pub fn jsonStringifyWithOptions(self: @This(), allocator: std.mem.Allocator, writer: *std.Io.Writer, options: @This().JsonStringifyOptions) !void {
                    ...
                    var first = true;
                    if (self.direct_leaf) |_pbz_nested| {
                        try writer.writeAll(if (options.preserve_proto_field_names) (if (first) "\"direct_leaf\":" else ",\"direct_leaf\":") else (if (first) "\"directLeaf\":" else ",\"directLeaf\":")); first = false;
                    ...
                    if (self.mid) |_pbz_nested| {
                        try writer.writeAll(if (first) "\"mid\":" else ",\"mid\":"); first = false;
                        try _pbz_nested.jsonStringifyWithOptions(allocator, writer, .{ .enum_as_name = options.enum_as_name, .preserve_proto_field_names = options.preserve_proto_field_names, .always_print_primitive_fields = options.always_print_primitive_fields });
                    }
                    try writer.writeAll("}");
                }
                ...
                fn jsonFillFromValue(self: *@This(), allocator: std.mem.Allocator, arena_allocator: std.mem.Allocator, json_value: std.json.Value, options: @This().JsonParseOptions) !void {
                    ...
                    while (it.next()) |entry| {
                        ...
                        if (value == .null) {
                            if (std.mem.eql(u8, key, "direct_leaf") or std.mem.eql(u8, key, "directLeaf")) {
                                if (self.direct_leaf) |*old_value| old_value.deinit(allocator); self.direct_leaf = null;
                                continue;
                            }
                            if (std.mem.eql(u8, key, "mid")) {
                                if (self.mid) |*old_value| old_value.deinit(allocator); self.mid = null;
                                continue;
                            }
                        ...
                        if (std.mem.eql(u8, key, "direct_leaf") or std.mem.eql(u8, key, "directLeaf")) {
                            var _pbz_nested = try imports.public_mid_proto.imports.public_leaf_proto.demo.publics.leaf.Leaf.jsonParseValueWithOptions(allocator, arena_allocator, value, .{ .ignore_unknown_fields = options.ignore_unknown_fields });
                            errdefer _pbz_nested.deinit(allocator);
                            if (self.direct_leaf) |*existing| { try existing.mergeFrom(allocator, _pbz_nested); _pbz_nested.deinit(allocator); } else { self.direct_leaf = _pbz_nested; }
                            continue;
                        }
                        if (std.mem.eql(u8, key, "mid")) {
                            var _pbz_nested = try imports.public_mid_proto.demo.publics.mid.Mid.jsonParseValueWithOptions(allocator, arena_allocator, value, .{ .ignore_unknown_fields = options.ignore_unknown_fields });
                            errdefer _pbz_nested.deinit(allocator);
                            if (self.mid) |*existing| { try existing.mergeFrom(allocator, _pbz_nested); _pbz_nested.deinit(allocator); } else { self.mid = _pbz_nested; }
                            continue;
                        }
                        if (options.ignore_unknown_fields) continue;
                        return error.UnknownField;
...
fn jsonWriteString(writer: *std.Io.Writer, value: []const u8) !void {
    ...
    try std.json.Stringify.value(value, .{}, writer);
                ...
                pub fn formatTextWithOptions(self: @This(), allocator: std.mem.Allocator, writer: *std.Io.Writer, options: @This().TextFormatOptions) !void {
                    if (self.direct_leaf) |_pbz_nested| {
                        try writer.writeAll("direct_leaf {\n");
                ...
                pub fn parseTextWithOptions(allocator: std.mem.Allocator, text: []const u8, options: @This().TextParseOptions) !@This() {
                    ...
                    while (lines.next()) |raw_line| {
                        const line = @This().textCleanLine(raw_line, text_has_comments);
                        if (line.len == 0) continue;
                        if (@This().textBlockField(line, "direct_leaf") or @This().textBlockField(line, "directLeaf")) {
                            const block = try @This().textBlock(allocator, &lines, text_has_comments);
                            defer allocator.free(block);
                            var _pbz_nested = try imports.public_mid_proto.imports.public_leaf_proto.demo.publics.leaf.Leaf.parseTextWithOptions(allocator, block, .{ .ignore_unknown_fields = options.ignore_unknown_fields });
                            if (self.direct_leaf) |*existing| { defer _pbz_nested.deinit(allocator); try existing.mergeFrom(allocator, _pbz_nested); } else { errdefer _pbz_nested.deinit(allocator); self.direct_leaf = _pbz_nested; }
                            continue;
                        }
                        if (@This().textBlockField(line, "mid")) {
                            const block = try @This().textBlock(allocator, &lines, text_has_comments);
                            defer allocator.free(block);
                            var _pbz_nested = try imports.public_mid_proto.demo.publics.mid.Mid.parseTextWithOptions(allocator, block, .{ .ignore_unknown_fields = options.ignore_unknown_fields });
                            if (self.mid) |*existing| { defer _pbz_nested.deinit(allocator); try existing.mergeFrom(allocator, _pbz_nested); } else { errdefer _pbz_nested.deinit(allocator); self.mid = _pbz_nested; }
                            continue;
                        }
