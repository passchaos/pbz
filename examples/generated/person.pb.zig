...
pub const Person = struct {
    ...
    pub fn hasField_id(self: @This()) bool {
        return self.id != 0;
    }

    pub fn getField_id(self: @This()) i32 {
        return self.id;
    }

    pub fn getOrDefaultField_id(self: @This()) i32 {
        return self.id;
    }

    pub fn setField_id(self: *@This(), value: i32) void {
        self.id = value;
    }

    pub fn clearField_id(self: *@This()) void {
        self.id = 0;
    }

    pub fn hasField_name(self: @This()) bool {
        return self.name.len != 0;
    }

    pub fn getField_name(self: @This()) []const u8 {
        return self.name;
    }

    pub fn getOrDefaultField_name(self: @This()) []const u8 {
        return self.name;
    }

    pub fn setField_name(self: *@This(), value: []const u8) void {
        self.name = value;
    }

    pub fn clearField_name(self: *@This()) void {
        self.name = "";
    }

    pub fn hasField_scores(self: @This()) bool {
        return self.scores.len != 0;
    }

    pub fn getField_scores(self: @This()) []const i32 {
        return self.scores;
    }

    pub fn appendField_scores(self: *@This(), allocator: std.mem.Allocator, value: i32) !void {
        const old = self.scores;
        const merged = try allocator.alloc(i32, old.len + 1);
        if (old.len != 0) @memcpy(merged[0..old.len], old);
        merged[old.len] = value;
        self.scores = merged;
        if (old.len != 0) allocator.free(old);
    }

    pub fn appendAllField_scores(self: *@This(), allocator: std.mem.Allocator, values: []const i32) !void {
        if (values.len == 0) return;
        const old = self.scores;
        const merged = try allocator.alloc(i32, old.len + values.len);
        if (old.len != 0) @memcpy(merged[0..old.len], old);
        @memcpy(merged[old.len..], values);
        self.scores = merged;
        if (old.len != 0) allocator.free(old);
    }

    pub fn replaceField_scores(self: *@This(), allocator: std.mem.Allocator, values: []const i32) !void {
        const old = self.scores;
        if (values.len == 0) {
            self.scores = &.{};
            if (old.len != 0) allocator.free(old);
        ...
        const owned = try allocator.dupe(i32, values);
        self.scores = owned;
        if (old.len != 0) allocator.free(old);
    }

    pub fn clearField_scores(self: *@This(), allocator: std.mem.Allocator) void {
        const old = self.scores;
        self.scores = &.{};
        if (old.len != 0) allocator.free(old);
    }

    pub fn hasField_counts(self: @This()) bool {
        return self.counts.len != 0;
    }

    pub fn getField_counts(self: @This()) []const countsEntry {
        return self.counts;
    }

    pub fn appendField_counts(self: *@This(), allocator: std.mem.Allocator, value: countsEntry) !void {
        var list: std.ArrayList(countsEntry) = .empty;
        defer list.deinit(allocator);
        if (self.counts.len != 0) try list.appendSlice(allocator, self.counts);
        try @This().appendOrReplaceMapEntry_counts(allocator, &list, value);
        const old = self.counts;
        self.counts = try list.toOwnedSlice(allocator);
        if (old.len != 0) allocator.free(old);
    }

    pub fn appendAllField_counts(self: *@This(), allocator: std.mem.Allocator, values: []const countsEntry) !void {
        if (values.len == 0) return;
        var list: std.ArrayList(countsEntry) = .empty;
        defer list.deinit(allocator);
        if (self.counts.len != 0) try list.appendSlice(allocator, self.counts);
        for (values) |value| try @This().appendOrReplaceMapEntry_counts(allocator, &list, value);
        const old = self.counts;
        self.counts = try list.toOwnedSlice(allocator);
        if (old.len != 0) allocator.free(old);
    }

    pub fn replaceField_counts(self: *@This(), allocator: std.mem.Allocator, values: []const countsEntry) !void {
        const old = self.counts;
        if (values.len == 0) {
            self.counts = &.{};
            if (old.len != 0) allocator.free(old);
    ...
    pub fn clearField_counts(self: *@This(), allocator: std.mem.Allocator) void {
        const old = self.counts;
        self.counts = &.{};
        if (old.len != 0) allocator.free(old);
    }
    ...
    pub fn clearUnknownFieldsByNumber(self: *@This(), allocator: std.mem.Allocator, number: pbz.FieldNumber) !void {
        ...
        for (self._unknown_fields) |raw| {
            var r = pbz.Reader.init(raw);
            const tag = (try r.nextTag()) orelse {
                allocator.free(raw);
                continue;
            };
    ...
    pub fn mergeFrom(self: *@This(), allocator: std.mem.Allocator, other: @This()) !void {
        ...
        if (other.counts.len != 0) {
            for (other.counts) |entry| try self.appendField_counts(allocator, entry);
        }
    ...
    pub fn encodeDeterministic(self: @This(), allocator: std.mem.Allocator) ![]u8 {
        ...
        if (self.counts.len != 0) {
            ...
            std.mem.sort(countsEntry, entries, {}, struct {
                fn lessThan(_: void, a: countsEntry, b: countsEntry) bool {
    ...
    pub fn decode(allocator: std.mem.Allocator, bytes: []const u8) !@This() {
        ...
        errdefer {
            for (_unknown_fields_list.items) |raw| allocator.free(raw);
            _unknown_fields_list.deinit(allocator);
        }
                ...
                4 => {
                    ...
                    while (try entry_reader.nextTag()) |entry_tag| {
                        switch (entry_tag.number) {
                            1 => {
                                const value = try entry_reader.readBytes();
                    ...
                    if (skip_entry) {
                        ...
                        try unknown_writer.writeBytes(4, payload);
                        const raw = try allocator.dupe(u8, unknown_writer.slice());
    ...
    pub fn parseTextWithOptions(allocator: std.mem.Allocator, text: []const u8, options: TextParseOptions) !@This() {
        ...
        while (lines.next()) |raw_line| {
            ...
            if (@This().textFieldValue(line, "name")) |raw_value| {
                self.name = blk: {
                    const decoded = try @This().textUnquote(try self._pbzOwnedAllocator(allocator), raw_value);
            ...
            if (@This().textBlockField(line, "counts")) {
                ...
                while (lines.next()) |raw_entry_line| {
                    ...
                    if (@This().textFieldValue(entry_line, "key")) |raw_key| {
                        entry.key = blk: {
                            const decoded = try @This().textUnquote(try self._pbzOwnedAllocator(allocator), raw_key);
                            if (!std.unicode.utf8ValidateSlice(decoded)) return error.InvalidUtf8;
                            break :blk decoded;
                        };
                        continue;
                    }
                    if (@This().textFieldValue(entry_line, "value")) |raw_value| {
                        entry.value = try @This().textInt(i32, raw_value);
                        continue;
                    }
                    return error.UnknownField;
                }
            ...
            if (try @This().textUnknownGroup(allocator, line, &lines)) |raw| {
                errdefer allocator.free(raw);
                try _unknown_fields_list.append(allocator, raw);
                continue;
            }
