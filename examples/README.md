...
- `reflection_facade.zig` — reflection facade over registry descriptors and
  ...
  typed message/group creation/access/mutation, maps, map key/value metadata, map key lookup, repeated/map enum-name lookup, JSON-name field lookup/output, field presence/packing/reserved
