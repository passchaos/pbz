...
- `reflection_facade.zig` — reflection facade over registry descriptors and
  ...
  typed singular/repeated message/group creation/access/mutation including typed repeated element access, field scalar/kind/cardinality/optional-keyword/weak/lazy/wire-type/encoded-wire-type/containing/extendee/oneof metadata, maps, map-entry descriptor metadata, map key/value metadata, map size/key/value/entry enumeration, map key lookup, repeated/map enum-name lookup, JSON-name field lookup/output/explicitness, explicit/effective default metadata and enum default value/name lookup, field presence/packing/reserved range/name enumeration/accessors
