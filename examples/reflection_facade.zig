...
pub fn main() !void {
    ...
    const role_unknown_direct_full_name = try refl.enumValueDirectFullName(role_unknown_value);
    defer allocator.free(role_unknown_direct_full_name);
    try std.testing.expectEqualStrings("demo.reflect.ROLE_UNKNOWN", role_unknown_direct_full_name);
    ...
    const deprecated_option = try refl.optionAt(id_options, 0);
    try std.testing.expectEqualStrings("deprecated", refl.optionName(deprecated_option));
    try std.testing.expectEqualStrings("deprecated", refl.optionLeafName(deprecated_option));
    const deprecated_value = refl.optionRawValue(deprecated_option);
    try std.testing.expectEqual(.boolean, refl.optionValueTag(deprecated_value));
    try std.testing.expect(refl.optionValueBool(deprecated_value).?);
    const note_option = try refl.optionAt(id_options, 1);
    try std.testing.expectEqualStrings("(demo.note)", refl.optionName(note_option));
    try std.testing.expectEqualStrings("note", refl.optionLeafName(note_option));
    const note_value = refl.optionRawValue(note_option);
    try std.testing.expectEqual(.string, refl.optionValueTag(note_value));
    try std.testing.expectEqualStrings("primary id", refl.optionValueString(note_value).?);
    try std.testing.expectEqualStrings("primary id", refl.optionValueIdentifier(note_value).?);
    const answer_value = refl.optionRawValue(try refl.optionAt(id_options, 2));
    try std.testing.expectEqual(.integer, refl.optionValueTag(answer_value));
    try std.testing.expectEqual(@as(i64, 42), refl.optionValueInteger(answer_value).?);
    try std.testing.expect(refl.optionValueString(answer_value) == null);
    const max_u64_value = refl.optionRawValue(try refl.optionAt(id_options, 3));
    try std.testing.expectEqual(.unsigned_integer, refl.optionValueTag(max_u64_value));
    try std.testing.expectEqual(@as(u64, 18446744073709551615), refl.optionValueUnsignedInteger(max_u64_value).?);
    const ratio_value = refl.optionRawValue(try refl.optionAt(id_options, 4));
    try std.testing.expectEqual(.float, refl.optionValueTag(ratio_value));
    try std.testing.expectEqual(@as(f64, 1.5), refl.optionValueFloat(ratio_value).?);
    const meta_value = refl.optionRawValue(try refl.optionAt(id_options, 5));
    try std.testing.expectEqual(.aggregate, refl.optionValueTag(meta_value));
    try std.testing.expect(std.mem.indexOf(u8, refl.optionValueAggregate(meta_value).?, "label") != null);
    ...
    try std.testing.expectEqual(@as(i32, 0), try refl.defaultValueInt32(id_default));
    try std.testing.expectError(error.TypeMismatch, refl.defaultValueBool(id_default));
    ...
    try std.testing.expectEqual(@as(u64, 0), try refl.defaultValueUInt64(try refl.fieldDefaultValue(user_desc, total_field)));
    const ratio_default = try refl.fieldDefaultValue(user_desc, try refl.fieldByName(user_desc, "ratio"));
    try std.testing.expectEqual(@as(f32, 0), try refl.fieldDefaultFloat(user_desc, try refl.fieldByName(user_desc, "ratio")));
    try std.testing.expectEqual(@as(f32, 0), try refl.defaultValueFloat(ratio_default));
    const score_default = try refl.fieldDefaultValue(user_desc, try refl.fieldByName(user_desc, "score"));
    try std.testing.expectEqual(@as(f64, 0), try refl.fieldDefaultDouble(user_desc, try refl.fieldByName(user_desc, "score")));
    try std.testing.expectEqual(@as(f64, 0), try refl.defaultValueDouble(score_default));
    const disabled_default = try refl.fieldDefaultValue(user_desc, try refl.fieldByName(user_desc, "disabled"));
    try std.testing.expectEqual(false, try refl.fieldDefaultBool(user_desc, try refl.fieldByName(user_desc, "disabled")));
    try std.testing.expectEqual(false, try refl.defaultValueBool(disabled_default));
    ...
    const tags_default = try refl.fieldDefaultValue(user_desc, tags_field);
    try std.testing.expectEqual(.none, refl.defaultValueTag(tags_default));
    try std.testing.expectEqual(pbz.dynamic.DefaultValue.none, tags_default);
    ...
    try std.testing.expect(try refl.methodDirectInputType(get_method) == user_desc);
    try std.testing.expect(try refl.methodDirectOutputType(get_method) == user_desc);
    ...
    try std.testing.expectEqualStrings("red", (try refl.mapKeyAt(&user, "counts", 0)).string);
    ...
    try std.testing.expectEqual(@as(i32, 2), (try refl.mapValueAt(&user, "counts", 0)).int32);
    try std.testing.expectError(error.MissingField, refl.mapEntryAt(&user, "counts", 9));
    const missing_count_entry = pbz.dynamic.MapEntry{ .key = .{ .string = count_keys[0].string }, .value = .{ .int32 = 2 } };
    try std.testing.expectError(error.MissingField, refl.mapEntryIndex(&user, "counts", &missing_count_entry));
    try std.testing.expectError(error.MissingField, refl.mapKeyAt(&user, "counts", 9));
    try std.testing.expectError(error.MissingField, refl.mapValueAt(&user, "counts", 9));
    ...
    try std.testing.expectEqual(@as(usize, 0), try refl.mapKeyIndex(&user, "counts", .{ .string = contains_red_key }));
    ...
    try std.testing.expectEqual(@as(usize, 0), try refl.mapValueIndex(&user, "counts", .{ .int32 = 2 }));
    const missing_key_lookup = try allocator.dupe(u8, "missing");
    defer allocator.free(missing_key_lookup);
    try std.testing.expectError(error.MissingField, refl.mapKeyIndex(&user, "counts", .{ .string = missing_key_lookup }));
    try std.testing.expectError(error.MissingField, refl.mapValueIndex(&user, "counts", .{ .int32 = 3 }));
    ...
    try std.testing.expectEqual(@as(pbz.FieldNumber, 100), try refl.unknownFieldNumberAt(&decoded, 0));
    try std.testing.expectEqual(@as(usize, 0), try refl.unknownFieldNumberIndex(&decoded, 100));
    try std.testing.expectError(error.UnknownField, refl.unknownFieldNumberAt(&decoded, 1));
    try std.testing.expectError(error.UnknownField, refl.unknownFieldNumberIndex(&decoded, 101));
    ...
    const unknown_run = try refl.unknownFieldNumberRunAt(&decoded, 0);
    try std.testing.expectEqual(@as(pbz.FieldNumber, 100), refl.unknownFieldNumberRunNumber(unknown_run));
    try std.testing.expectEqual(@as(usize, 1), refl.unknownFieldNumberRunCount(unknown_run));
    try std.testing.expectEqual(@as(usize, 0), try refl.unknownFieldNumberRunIndex(&decoded, 100));
    try std.testing.expectError(error.UnknownField, refl.unknownFieldNumberRunAt(&decoded, 1));
    try std.testing.expectError(error.UnknownField, refl.unknownFieldNumberRunIndex(&decoded, 101));
    ...
    try std.testing.expectEqual(@as(u64, 1), try refl.unknownFieldVarint(unknown_field));
    try std.testing.expectError(error.InvalidWireType, refl.unknownFieldFixed32(unknown_field));
    var fixed32_unknown_raw = pbz.Writer.init(allocator);
    defer fixed32_unknown_raw.deinit();
    try fixed32_unknown_raw.writeFixed32(101, 0xdead_beef);
    try refl.appendUnknownRaw(&decoded, fixed32_unknown_raw.slice());
    const fixed32_unknown = try refl.unknownAt(&decoded, 1);
    try std.testing.expectEqual(@as(u32, 0xdead_beef), try refl.unknownFieldFixed32(fixed32_unknown));
    var fixed64_unknown_raw = pbz.Writer.init(allocator);
    defer fixed64_unknown_raw.deinit();
    try fixed64_unknown_raw.writeFixed64(102, 0x0102_0304_0506_0708);
    try refl.appendUnknownRaw(&decoded, fixed64_unknown_raw.slice());
    const fixed64_unknown = try refl.unknownAt(&decoded, 2);
    try std.testing.expectEqual(@as(u64, 0x0102_0304_0506_0708), try refl.unknownFieldFixed64(fixed64_unknown));
    var bytes_unknown_raw = pbz.Writer.init(allocator);
    defer bytes_unknown_raw.deinit();
    try bytes_unknown_raw.writeBytes(103, "opaque");
    try refl.appendUnknownRaw(&decoded, bytes_unknown_raw.slice());
    const bytes_unknown = try refl.unknownAt(&decoded, 3);
    try std.testing.expectEqualStrings("opaque", try refl.unknownFieldLengthDelimited(bytes_unknown));
    var group_unknown_raw = pbz.Writer.init(allocator);
    defer group_unknown_raw.deinit();
    try group_unknown_raw.writeTag(104, .start_group);
    try group_unknown_raw.writeUInt32(1, 7);
    try group_unknown_raw.writeTag(104, .end_group);
    try refl.appendUnknownRaw(&decoded, group_unknown_raw.slice());
    const group_unknown = try refl.unknownAt(&decoded, 4);
    try std.testing.expectEqualSlices(u8, &.{ 0x08, 0x07 }, try refl.unknownFieldGroupPayload(group_unknown));
    try std.testing.expectError(error.UnknownField, refl.unknownAt(&decoded, 5));
    ...
    try std.testing.expectEqual(@as(usize, 4), refl.unknownCount(&decoded));
    try std.testing.expect(!refl.hasUnknownFieldNumber(&decoded, 100));
    ...
    defer allocator.free(contact_descriptor_fields);
    try std.testing.expectEqual(@as(usize, 2), contact_fields.len);
    try std.testing.expectEqual(@as(usize, 2), contact_descriptor_fields.len);
    try std.testing.expectEqual(@as(usize, 2), try refl.oneofFieldCount(user_desc, "contact"));
    try std.testing.expectEqual(@as(usize, 2), try refl.oneofDescriptorFieldCount(user_desc, contact_oneof));
    ...
    try std.testing.expect(contact_descriptor_fields[0] == contact_fields[0]);
    try std.testing.expect(contact_descriptor_fields[1] == contact_fields[1]);
