...
- `google.protobuf.Any` with embedded well-known-type JSON values, including
  zero/explicit-plus/positive/micro/nano/integer-negative/fractional-negative/min-max-bound `Duration`, camel-case and empty `FieldMask`, micro/nano/offset/min/pre/post/max-bound `Timestamp`, canonical `Empty`, non-empty/empty `Struct`, object/list/scalar-valued `Value` (including default-like scalar and empty object/list kinds), 64-bit min/zero/positive/negative/max `Int64Value`, negative/zero/positive finite `DoubleValue`, non-finite `DoubleValue` (`NaN`, `Infinity`, `-Infinity`), negative/zero/positive finite `FloatValue`, non-finite `FloatValue` (`NaN`, `Infinity`, `-Infinity`), min/zero/positive/negative/max `Int32Value`, zero/normal/max unsigned `UInt32Value`, zero/normal/max unsigned `UInt64Value`, true/false `BoolValue`, non-empty/empty `StringValue`, base64/empty `BytesValue`, and recursive nested `Any`, stringify/parse
