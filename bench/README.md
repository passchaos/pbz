...
- `google.protobuf.Any` with embedded well-known-type JSON values, including
  `Duration`, camel-case `FieldMask`, instant `Timestamp`, object-subtree `Struct`, object-valued `Value`, scalar-wrapper `StringValue`, base64 `BytesValue`, and recursive nested `Any`, stringify/parse
