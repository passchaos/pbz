...
is `/tmp/pbz-compare-open-enum-json-final.log` summarized by
`/tmp/pbz-summary-open-enum-json-final.txt`. The fail-on-loss summary gate:

```sh
python3 bench/summarize_compare.py /tmp/pbz-compare-open-enum-json-final.log --fail-on-loss
```
...
`bench/summarize_compare.py` currently tracks 400 workloads. The parsed baselines
include:
...
- simple JSON stringify/parse, including generated `map<string, int32>` surrogate-pair key parse, generated null-field parse, generated open-enum numeric parse, and generated integer numeric-exponent token parse
