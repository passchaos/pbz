...
is `/tmp/pbz-compare-scalar-value-json-isolated.log` summarized by
`/tmp/pbz-summary-scalar-value-json-isolated.txt`. The fail-on-loss summary gate:

```sh
python3 bench/summarize_compare.py /tmp/pbz-compare-scalar-value-json-isolated.log --fail-on-loss
```
...
`bench/summarize_compare.py` currently tracks 270 workloads. The parsed baselines
include:
...
- direct `google.protobuf.Empty`, `Struct`, object/scalar `Value`, and `ListValue` JSON stringify/parse
