...
is `/tmp/pbz-compare-key-surrogate-json-final.log` summarized by
`/tmp/pbz-summary-key-surrogate-json-final.txt`. The fail-on-loss summary gate:

```sh
python3 bench/summarize_compare.py /tmp/pbz-compare-key-surrogate-json-final.log --fail-on-loss
```
...
`bench/summarize_compare.py` currently tracks 396 workloads. The parsed baselines
include:
