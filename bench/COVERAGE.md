...
is `/tmp/pbz-compare-value-string-escape-json-final.log` summarized by
`/tmp/pbz-summary-value-string-escape-json-final.txt`. The fail-on-loss summary gate:

```sh
python3 bench/summarize_compare.py /tmp/pbz-compare-value-string-escape-json-final.log --fail-on-loss
```
...
`bench/summarize_compare.py` currently tracks 352 workloads. The parsed baselines
include:
...
- direct `google.protobuf.Empty`, non-empty/escaped-input parse/empty `Struct`, object/escaped-object parse/list/string-scalar/escaped-string-scalar `Value` (including default-like scalar and empty object/list kinds), and non-empty/empty `ListValue` JSON stringify/parse
