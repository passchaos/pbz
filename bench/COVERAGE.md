...
- `examples/dynamic_defaults.zig` covers dynamic proto2 default values,
  ...
  through a registry.
- `examples/dynamic_enum_alias.zig` covers dynamic enum alias parsing and
  canonical JSON/TextFormat output for singular, repeated, and map enum fields.
...
- `examples/reflection_facade.zig` covers the public reflection facade over
  ...
  type-checked writes, repeated fields, map last-wins mutation, file/import-chain
  lookup, enum descriptor/value lookup, oneof lookup/inspection/clearing,
  imported message fields, service descriptor/method lookup, extension descriptor
