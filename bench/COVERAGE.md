...
is `/tmp/pbz-compare-after-bytesvalue-json.log` summarized by
`/tmp/pbz-summary-after-bytesvalue-json.txt`. The fail-on-loss summary gate:

```sh
python3 bench/summarize_compare.py /tmp/pbz-compare-after-bytesvalue-json.log --fail-on-loss
```
...
`bench/summarize_compare.py` currently tracks 66 workloads. The parsed baselines
include:
...
- direct `google.protobuf.StringValue` JSON stringify/parse
- direct `google.protobuf.BytesValue` JSON stringify/parse
