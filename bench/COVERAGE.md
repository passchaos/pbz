...
is `/tmp/pbz-compare-after-min-duration-json-isolated.log` summarized by
`/tmp/pbz-summary-after-min-duration-json-isolated.txt`. The fail-on-loss summary gate:

```sh
python3 bench/summarize_compare.py /tmp/pbz-compare-after-min-duration-json-isolated.log --fail-on-loss
```
...
`bench/summarize_compare.py` currently tracks 170 workloads. The parsed baselines
include:
...
- direct positive, integer-negative, fractional-negative, min-bound, and max-bound `google.protobuf.Duration` JSON stringify/parse
