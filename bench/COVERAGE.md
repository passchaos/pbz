...
is `/tmp/pbz-compare-after-any-double-json-isolated.log` summarized by
`/tmp/pbz-summary-after-any-double-json-isolated.txt`. The fail-on-loss summary gate:

```sh
python3 bench/summarize_compare.py /tmp/pbz-compare-after-any-double-json-isolated.log --fail-on-loss
```
...
`bench/summarize_compare.py` currently tracks 112 workloads. The parsed baselines
include:
...
- `google.protobuf.Any` containing a well-known-type JSON value stringify/parse,
  including embedded `Struct`, `Value`, camel-case `FieldMask`, instant `Timestamp`, canonical `Empty`, 64-bit `Int64Value`, floating `DoubleValue`, unsigned `UInt64Value`, boolean `BoolValue`, scalar-wrapper `StringValue`, base64 `BytesValue`, and recursive nested `Any` payloads
