...
is `/tmp/pbz-compare-list-escape-json-final.log` summarized by
`/tmp/pbz-summary-list-escape-json-final.txt`. The fail-on-loss summary gate:

```sh
python3 bench/summarize_compare.py /tmp/pbz-compare-list-escape-json-final.log --fail-on-loss
```
...
`bench/summarize_compare.py` currently tracks 355 workloads. The parsed baselines
include:
