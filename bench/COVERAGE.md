...
is `/tmp/pbz-compare-negative-number-value-json-final.log` summarized by
`/tmp/pbz-summary-negative-number-value-json-final.txt`. The fail-on-loss summary gate:

```sh
python3 bench/summarize_compare.py /tmp/pbz-compare-negative-number-value-json-final.log --fail-on-loss
```
...
`bench/summarize_compare.py` currently tracks 365 workloads. The parsed baselines
include:
