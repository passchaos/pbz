...
is `/tmp/pbz-compare-short-fraction-duration-json-isolated.log` summarized by
`/tmp/pbz-summary-short-fraction-duration-json-isolated.txt`. The fail-on-loss summary gate:

```sh
python3 bench/summarize_compare.py /tmp/pbz-compare-short-fraction-duration-json-isolated.log --fail-on-loss
```
...
`bench/summarize_compare.py` currently tracks 322 workloads. The parsed baselines
include:
