...
is `/tmp/pbz-compare-after-any-fieldmask-json-isolated-rerun.log` summarized by
`/tmp/pbz-summary-after-any-fieldmask-json-isolated-rerun.txt`. The fail-on-loss summary gate:

```sh
python3 bench/summarize_compare.py /tmp/pbz-compare-after-any-fieldmask-json-isolated-rerun.log --fail-on-loss
```
...
`bench/summarize_compare.py` currently tracks 100 workloads. The parsed baselines
include:
...
- `google.protobuf.Any` containing a well-known-type JSON value stringify/parse,
  including embedded `Struct`, `Value`, camel-case `FieldMask`, scalar-wrapper `StringValue`, base64 `BytesValue`, and recursive nested `Any` payloads
