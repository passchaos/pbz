...
is `/tmp/pbz-compare-number-wrapper-json-isolated.log` summarized by
`/tmp/pbz-summary-number-wrapper-json-isolated.txt`. The fail-on-loss summary gate:

```sh
python3 bench/summarize_compare.py /tmp/pbz-compare-number-wrapper-json-isolated.log --fail-on-loss
```
...
`bench/summarize_compare.py` currently tracks 326 workloads. The parsed baselines
include:
...
- `google.protobuf.Any` containing a well-known-type JSON value stringify/parse,
  including embedded zero/explicit-plus parse/short-fraction parse/positive/micro/nano/integer-negative/fractional-negative/min-max-bound `Duration`, non-empty and empty `Struct`, object/list/scalar `Value` (including default-like scalar and empty object/list kinds), camel-case and empty `FieldMask`, micro/nano/offset/min/pre/post/max-bound `Timestamp`, canonical `Empty`, 64-bit min/zero/positive/number-input parse/negative/max `Int64Value`, negative/zero/positive finite `DoubleValue`, non-finite `DoubleValue` (`NaN`, `Infinity`, `-Infinity`), negative/zero/positive finite `FloatValue`, non-finite `FloatValue` (`NaN`, `Infinity`, `-Infinity`), min/zero/positive/negative/max `Int32Value`, zero/normal/max unsigned `UInt32Value`, zero/normal/number-input parse/max unsigned `UInt64Value`, true/false `BoolValue`, non-empty/empty `StringValue`, base64/empty `BytesValue`, and recursive nested `Any` payloads
...
- direct micro, nano, timezone-offset parse, min-bound, pre-epoch, post-epoch, and max-bound `google.protobuf.Timestamp` JSON stringify/parse
...
- direct scalar wrapper JSON stringify/parse for `DoubleValue`, `FloatValue`,
  min/zero/positive/number-input parse/negative/max `Int64Value`, zero/normal/number-input parse/max `UInt64Value`, min/zero/positive/negative/max `Int32Value`, zero/normal/max `UInt32Value`, true/false `BoolValue`, plus negative/zero/positive finite and non-finite `DoubleValue` and `FloatValue` (`NaN`, `Infinity`, `-Infinity`),
