...
is `/tmp/pbz-compare-empty-struct-list-json-isolated.log` summarized by
`/tmp/pbz-summary-empty-struct-list-json-isolated.txt`. The fail-on-loss summary gate:

```sh
python3 bench/summarize_compare.py /tmp/pbz-compare-empty-struct-list-json-isolated.log --fail-on-loss
```
...
`bench/summarize_compare.py` currently tracks 276 workloads. The parsed baselines
include:
...
- direct `google.protobuf.Empty`, non-empty/empty `Struct`, object/scalar `Value`, and non-empty/empty `ListValue` JSON stringify/parse
