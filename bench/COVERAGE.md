...
is `/tmp/pbz-compare-fractional-time-precision-json-isolated.log` summarized by
`/tmp/pbz-summary-fractional-time-precision-json-isolated.txt`. The fail-on-loss summary gate:

```sh
python3 bench/summarize_compare.py /tmp/pbz-compare-fractional-time-precision-json-isolated.log --fail-on-loss
```
...
`bench/summarize_compare.py` currently tracks 316 workloads. The parsed baselines
include:
...
- direct micro, nano, min-bound, pre-epoch, post-epoch, and max-bound `google.protobuf.Timestamp` JSON stringify/parse
