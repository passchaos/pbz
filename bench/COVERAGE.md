...
is `/tmp/pbz-compare-value-surrogate-json-final.log` summarized by
`/tmp/pbz-summary-value-surrogate-json-final.txt`. The fail-on-loss summary gate:

```sh
python3 bench/summarize_compare.py /tmp/pbz-compare-value-surrogate-json-final.log --fail-on-loss
```
...
`bench/summarize_compare.py` currently tracks 385 workloads. The parsed baselines
include:
