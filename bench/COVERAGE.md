...
is `/tmp/pbz-compare-signed-minmax-wrapper-json-isolated.log` summarized by
`/tmp/pbz-summary-signed-minmax-wrapper-json-isolated.txt`. The fail-on-loss summary gate:

```sh
python3 bench/summarize_compare.py /tmp/pbz-compare-signed-minmax-wrapper-json-isolated.log --fail-on-loss
```
...
`bench/summarize_compare.py` currently tracks 242 workloads. The parsed baselines
include:
...
- `google.protobuf.Any` containing a well-known-type JSON value stringify/parse,
  including embedded zero/positive/integer-negative/fractional-negative/min-max-bound `Duration`, `Struct`, `Value`, camel-case `FieldMask`, min/pre/post/max-bound `Timestamp`, canonical `Empty`, 64-bit min/zero/positive/negative/max `Int64Value`, zero/finite `DoubleValue`, non-finite `DoubleValue` (`NaN`, `Infinity`, `-Infinity`), zero/finite `FloatValue`, non-finite `FloatValue` (`NaN`, `Infinity`, `-Infinity`), min/zero/positive/negative/max `Int32Value`, zero/normal/max unsigned `UInt32Value`, zero/normal/max unsigned `UInt64Value`, true/false `BoolValue`, non-empty/empty `StringValue`, base64/empty `BytesValue`, and recursive nested `Any` payloads
...
- direct scalar wrapper JSON stringify/parse for `DoubleValue`, `FloatValue`,
  min/zero/positive/negative/max `Int64Value`, zero/normal/max `UInt64Value`, min/zero/positive/negative/max `Int32Value`, zero/normal/max `UInt32Value`, true/false `BoolValue`, plus zero and non-finite `DoubleValue` and `FloatValue` (`NaN`, `Infinity`, `-Infinity`),
