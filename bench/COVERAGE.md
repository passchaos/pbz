...
- `examples/reflection_facade.zig` covers the public reflection facade over
  ...
  message/enum legacy-json-conflict metadata, feature-support lifecycle metadata/accessors/presence, ordered file/message/nested-type/enum/service descriptor enumeration/index lookup, file/message-local descriptor/value lookup/index, descriptor name/full-name/containing-type/value-owner/direct field-type identity and placeholder metadata lookup,
...
- `examples/well_known_types.zig` covers selected well known types, including
  Timestamp, Duration, FieldMask path presence/access/index, Empty, Struct field presence/access/index, Value equality/typed accessors, ListValue presence/access/index, scalar and
...
- `examples/any_type_url.zig` covers custom `Any` type URL prefixes/accessors/matching, leading-dot
