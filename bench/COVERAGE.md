...
is `/tmp/pbz-compare-struct-value-escape-json-isolated.log` summarized by
`/tmp/pbz-summary-struct-value-escape-json-isolated.txt`. The fail-on-loss summary gate:

```sh
python3 bench/summarize_compare.py /tmp/pbz-compare-struct-value-escape-json-isolated.log --fail-on-loss
```
...
`bench/summarize_compare.py` currently tracks 350 workloads. The parsed baselines
include:
...
- direct `google.protobuf.Empty`, non-empty/escaped-input parse/empty `Struct`, object/escaped-object parse/list/scalar `Value` (including default-like scalar and empty object/list kinds), and non-empty/empty `ListValue` JSON stringify/parse
