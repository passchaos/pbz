...
is `/tmp/pbz-compare-after-max-timestamp-json-isolated.log` summarized by
`/tmp/pbz-summary-after-max-timestamp-json-isolated.txt`. The fail-on-loss summary gate:

```sh
python3 bench/summarize_compare.py /tmp/pbz-compare-after-max-timestamp-json-isolated.log --fail-on-loss
```
...
`bench/summarize_compare.py` currently tracks 154 workloads. The parsed baselines
include:
...
- `google.protobuf.Any` containing a well-known-type JSON value stringify/parse,
  including embedded positive/negative `Duration`, `Struct`, `Value`, camel-case `FieldMask`, post/pre-epoch and max-bound `Timestamp`, canonical `Empty`, 64-bit `Int64Value`, floating `DoubleValue`, non-finite `DoubleValue` (`NaN`, `Infinity`, `-Infinity`), `FloatValue`, non-finite `FloatValue` (`NaN`, `Infinity`, `-Infinity`), `Int32Value`, unsigned `UInt32Value`, unsigned `UInt64Value`, boolean `BoolValue`, scalar-wrapper `StringValue`, base64 `BytesValue`, and recursive nested `Any` payloads
...
- direct post-epoch, pre-epoch, and max-bound `google.protobuf.Timestamp` JSON stringify/parse
