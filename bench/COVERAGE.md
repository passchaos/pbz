...
is `/tmp/pbz-compare-default-value-scalars-json-isolated.log` summarized by
`/tmp/pbz-summary-default-value-scalars-json-isolated.txt`. The fail-on-loss summary gate:

```sh
python3 bench/summarize_compare.py /tmp/pbz-compare-default-value-scalars-json-isolated.log --fail-on-loss
```
...
`bench/summarize_compare.py` currently tracks 300 workloads. The parsed baselines
include:
