...
is `/tmp/pbz-compare-after-unknown-sidecar.log` summarized by
`/tmp/pbz-summary-after-unknown-sidecar.txt`. The fail-on-loss summary gate:

```sh
python3 bench/summarize_compare.py /tmp/pbz-compare-after-unknown-sidecar.log --fail-on-loss
```
