...
is `/tmp/pbz-compare-string-surrogate-json-final.log` summarized by
`/tmp/pbz-summary-string-surrogate-json-final.txt`. The fail-on-loss summary gate:

```sh
python3 bench/summarize_compare.py /tmp/pbz-compare-string-surrogate-json-final.log --fail-on-loss
```
...
`bench/summarize_compare.py` currently tracks 383 workloads. The parsed baselines
include:
