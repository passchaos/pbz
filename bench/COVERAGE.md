...
is `/tmp/pbz-compare-ignore-unknown-json-final.log` summarized by
`/tmp/pbz-summary-ignore-unknown-json-final.txt`. The fail-on-loss summary gate:

```sh
python3 bench/summarize_compare.py /tmp/pbz-compare-ignore-unknown-json-final.log --fail-on-loss
```
...
`bench/summarize_compare.py` currently tracks 405 workloads. The parsed baselines
include:
