...
is `/tmp/pbz-compare-null-fields-json-final.log` summarized by
`/tmp/pbz-summary-null-fields-json-final.txt`. The fail-on-loss summary gate:

```sh
python3 bench/summarize_compare.py /tmp/pbz-compare-null-fields-json-final.log --fail-on-loss
```
...
`bench/summarize_compare.py` currently tracks 399 workloads. The parsed baselines
include:
...
- simple JSON stringify/parse, including generated `map<string, int32>` surrogate-pair key parse, generated null-field parse, and generated integer numeric-exponent token parse
