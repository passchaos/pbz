...
is `/tmp/pbz-compare-after-duration-json.log` summarized by
`/tmp/pbz-summary-after-duration-json.txt`. The fail-on-loss summary gate:

```sh
python3 bench/summarize_compare.py /tmp/pbz-compare-after-duration-json.log --fail-on-loss
```
...
`bench/summarize_compare.py` currently tracks 58 workloads. The parsed baselines
include:
