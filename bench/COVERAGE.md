...
is `/tmp/pbz-compare-float-string-wrapper-json-isolated.log` summarized by
`/tmp/pbz-summary-float-string-wrapper-json-isolated.txt`. The fail-on-loss summary gate:

```sh
python3 bench/summarize_compare.py /tmp/pbz-compare-float-string-wrapper-json-isolated.log --fail-on-loss
```
...
`bench/summarize_compare.py` currently tracks 334 workloads. The parsed baselines
include:
