...
is `/tmp/pbz-compare-after-direct-varint.log` summarized by
`/tmp/pbz-summary-after-direct-varint.txt`. The fail-on-loss summary gate:

```sh
python3 bench/summarize_compare.py /tmp/pbz-compare-after-direct-varint.log --fail-on-loss
```
