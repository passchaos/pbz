...
is `/tmp/pbz-compare-after-textlookup.log` summarized by
`/tmp/pbz-summary-after-textlookup.txt`. The fail-on-loss summary gate:

```sh
python3 bench/summarize_compare.py /tmp/pbz-compare-after-textlookup.log --fail-on-loss
```
