...
is `/tmp/pbz-compare-after-stringvalue-json.log` summarized by
`/tmp/pbz-summary-after-stringvalue-json.txt`. The fail-on-loss summary gate:

```sh
python3 bench/summarize_compare.py /tmp/pbz-compare-after-stringvalue-json.log --fail-on-loss
```
...
`bench/summarize_compare.py` currently tracks 64 workloads. The parsed baselines
include:
