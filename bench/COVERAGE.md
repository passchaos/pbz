...
is `/tmp/pbz-compare-temporal-escape-json-isolated-rerun.log` summarized by
`/tmp/pbz-summary-temporal-escape-json-isolated-rerun.txt`. The fail-on-loss summary gate:

```sh
python3 bench/summarize_compare.py /tmp/pbz-compare-temporal-escape-json-isolated-rerun.log --fail-on-loss
```
...
`bench/summarize_compare.py` currently tracks 346 workloads. The parsed baselines
include:
