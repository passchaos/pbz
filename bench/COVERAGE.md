...
is `/tmp/pbz-compare-bytes-unpadded-json-final.log` summarized by
`/tmp/pbz-summary-bytes-unpadded-json-final.txt`. The fail-on-loss summary gate:

```sh
python3 bench/summarize_compare.py /tmp/pbz-compare-bytes-unpadded-json-final.log --fail-on-loss
```
...
`bench/summarize_compare.py` currently tracks 379 workloads. The parsed baselines
include:
