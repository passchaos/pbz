...
is `/tmp/pbz-compare-int64-uint64-exponent-json-final.log` summarized by
`/tmp/pbz-summary-int64-uint64-exponent-json-final.txt`. The fail-on-loss summary gate:

```sh
python3 bench/summarize_compare.py /tmp/pbz-compare-int64-uint64-exponent-json-final.log --fail-on-loss
```
...
`bench/summarize_compare.py` currently tracks 377 workloads. The parsed baselines
include:
