...
is `/tmp/pbz-compare-after-any-empty-json-isolated-rerun2.log` summarized by
`/tmp/pbz-summary-after-any-empty-json-isolated-rerun2.txt`. The fail-on-loss summary gate:

```sh
python3 bench/summarize_compare.py /tmp/pbz-compare-after-any-empty-json-isolated-rerun2.log --fail-on-loss
```
...
`bench/summarize_compare.py` currently tracks 104 workloads. The parsed baselines
include:
...
- `google.protobuf.Any` containing a well-known-type JSON value stringify/parse,
  including embedded `Struct`, `Value`, camel-case `FieldMask`, instant `Timestamp`, canonical `Empty`, scalar-wrapper `StringValue`, base64 `BytesValue`, and recursive nested `Any` payloads
