...
is `/tmp/pbz-compare-bytes-standard-base64-json-final.log` summarized by
`/tmp/pbz-summary-bytes-standard-base64-json-final.txt`. The fail-on-loss summary gate:

```sh
python3 bench/summarize_compare.py /tmp/pbz-compare-bytes-standard-base64-json-final.log --fail-on-loss
```
...
`bench/summarize_compare.py` currently tracks 381 workloads. The parsed baselines
include:
