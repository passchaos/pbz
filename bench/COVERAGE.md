...
is `/tmp/pbz-compare-bytes-url-json-isolated.log` summarized by
`/tmp/pbz-summary-bytes-url-json-isolated.txt`. The fail-on-loss summary gate:

```sh
python3 bench/summarize_compare.py /tmp/pbz-compare-bytes-url-json-isolated.log --fail-on-loss
```
...
`bench/summarize_compare.py` currently tracks 338 workloads. The parsed baselines
include:
