...
is `/tmp/pbz-compare-default-wrapper-json-isolated.log` summarized by
`/tmp/pbz-summary-default-wrapper-json-isolated.txt`. The fail-on-loss summary gate:

```sh
python3 bench/summarize_compare.py /tmp/pbz-compare-default-wrapper-json-isolated.log --fail-on-loss
```
...
`bench/summarize_compare.py` currently tracks 202 workloads. The parsed baselines
include:
...
- `google.protobuf.Any` containing a well-known-type JSON value stringify/parse,
  including embedded zero/positive/integer-negative/fractional-negative/min-max-bound `Duration`, `Struct`, `Value`, camel-case `FieldMask`, min/pre/post/max-bound `Timestamp`, canonical `Empty`, 64-bit positive/negative `Int64Value`, floating `DoubleValue`, non-finite `DoubleValue` (`NaN`, `Infinity`, `-Infinity`), `FloatValue`, non-finite `FloatValue` (`NaN`, `Infinity`, `-Infinity`), positive/negative `Int32Value`, normal/max unsigned `UInt32Value`, normal/max unsigned `UInt64Value`, true/false `BoolValue`, non-empty/empty `StringValue`, base64/empty `BytesValue`, and recursive nested `Any` payloads
