...
is `/tmp/pbz-compare-after-min-timestamp-json-isolated.log` summarized by
`/tmp/pbz-summary-after-min-timestamp-json-isolated.txt`. The fail-on-loss summary gate:

```sh
python3 bench/summarize_compare.py /tmp/pbz-compare-after-min-timestamp-json-isolated.log --fail-on-loss
```
...
`bench/summarize_compare.py` currently tracks 158 workloads. The parsed baselines
include:
