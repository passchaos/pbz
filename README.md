...
- Protoc plugin and codegen helpers
  ...
  - Zig typed scalar/repeated-scalar/enum/message-payload/map skeleton with AST syntax validation generation, including proto `allow_alias` enum values emitted as Zig enum namespace aliases and generated enum `fromInt` / `toInt` / `protoName` / JSON stringify helpers
  ...
  - generated message structs expose field accessors for presence-aware singular fields, repeated/map append/replace/clear, oneof union arms, typed enum get/set/default/repeated-batch/map-entry helpers, typed map-message entry helpers, same-file typed message payload encode/decode helpers, decodeMessageField aliases, and `cloneOwned` / `decodeOwned` helpers for deep-copying decoded slice payloads
