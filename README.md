...
- Dynamic message runtime
  ...
  - descriptor encode/decode for common `FileOptions` fields, `MessageOptions.map_entry`, `FeatureSet` on file/message/field/oneof/enum/enum-value/service/method/extension-range options, `FeatureSetDefaults` with closest-edition lookup helpers, plus `FieldOptions.edition_defaults` and `FieldOptions.feature_support`, preserving protobuf edition feature metadata as structured schema fields, rejecting malformed decoded symbol identifiers/type references/defaults/json names, and using descriptor-compatible C escapes for proto2 bytes defaults
