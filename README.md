...
- Dynamic message runtime
  ...
  - editions `features.repeated_field_encoding` packed/expanded behavior, enum-level `features.enum_type` open/closed override behavior, string UTF-8 validation via `features.utf8_validation`, and implicit/explicit `features.field_presence` default-value encoding behavior
