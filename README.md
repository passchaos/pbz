...
- Well-known types
  - basic google.protobuf.Timestamp, Duration, FieldMask, Any, Empty, Struct/Value/ListValue, and wrapper wire/JSON parse/stringify helpers with validation plus dynamic JSON mapping, including canonical 0/3/6/9 fractional JSON output for Timestamp/Duration, registry-aware Any expanded payload JSON plus strict standalone Any object fields, strict Timestamp timezone-offset and malformed-number parsing, Duration sign/range validation, wrapper null/default parsing and float special values, strict FieldMask snake_case/lowerCamel JSON path validation, and strict Empty object parsing
- Conformance helpers
