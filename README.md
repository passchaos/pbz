...
- Conformance helpers
  - basic ConformanceRequest decode, safe enum handling, ConformanceResponse encode, and dynamic runner with deterministic registry-aware protobuf output including JSON/TextFormat input plus unknown-field/group preservation and TextFormat unknown-group output, registry-aware JSON/Text input and JSON ignore-unknown category handling, imported JSON/Text type conversion paths, proto2 extension and MessageSet JSON/TextFormat paths, and missing-required path parse errors
