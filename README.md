...
- Conformance helpers
  - basic ConformanceRequest decode, safe enum handling, ConformanceResponse encode, and dynamic runner with deterministic registry-aware protobuf output including JSON/TextFormat input, registry-aware JSON/Text input, imported JSON/Text type conversion paths, proto2 extension JSON/TextFormat input plus JSON/TextFormat output, and missing-required path parse errors
