...
- Protoc plugin and codegen helpers
  ...
  - generated message structs expose field accessors for presence-aware singular fields, repeated/map append/replace/clear, oneof union arms, typed enum get/set/default/repeated-batch/map-entry helpers, same-file typed message payload encode/decode helpers, decodeMessageField aliases, and `cloneOwned` / `decodeOwned` helpers for deep-copying decoded slice payloads
